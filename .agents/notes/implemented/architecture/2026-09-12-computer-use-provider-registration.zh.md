...
**Cua Driver** 是上游实现的名称。[MCP 提供方](../../../../packages/experimental/computer-use-cua-driver-mcp/README.zh.md)连接已安装的可执行文件。[原生提供方](../../../../packages/experimental/computer-use-cua-driver-native/README.zh.md)安装上游原生 npm 依赖。两者按实验性分组的默认发布策略保持私有，且均不默认启用。

各集成暴露上游工具目录。MCP 结果转换保留在 `dsh-mcp-client` 中，其基于回调的工具适配函数也转换原生 Cua Driver 结果。计算机操作服务不依赖该适配函数或任一提供方。

提供方卸载时保留注册，直到停止接收工具调用且自有工作和资源关闭。分组 Cordis effect 为此清理排序；独立 effect 可能并发清理。原生提供方通过 `tools/execute` 在原生调用和截图准入之间共享取消信号，同时保留执行标识，并将每个带 Agent 的调用路由到由 Agent ID 派生的具名 Cua Driver 会话。此机制分离分叉 DSH 会话的驱动状态，但不会克隆或预留宿主窗口。并发桌面工作流仍由调用方协调，因为提供方注册不拥有观察、操作和验证流程。
...
服务保持独立于实验性包。两个提供方包保持私有，因此挂载其一是源码 checkout 的组合选择。配置选择提供方，切换需要先卸载当前提供方。
