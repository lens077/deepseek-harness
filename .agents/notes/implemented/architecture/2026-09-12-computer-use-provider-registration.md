...
**Cua Driver** names the upstream implementation. The [MCP provider](../../../../packages/experimental/computer-use-cua-driver-mcp/README.md) connects an installed executable. The [native provider](../../../../packages/experimental/computer-use-cua-driver-native/README.md) installs the upstream native npm dependency. Both stay private under the experimental group's default publication policy, and neither is enabled by default.
...
The service remains independent of experimental packages. The two provider packages stay private, so mounting one is a source-checkout composition choice. Configuration selects a provider, and switching requires unloading the current provider first.
