...
`rev` 继续作为脚本 URL 的查询参数和内容一致性锚点，bundle 与 map 都以 `no-cache` 连同一个按所供给字节计算的实体标签一起供给，因此每次加载都会重校验，内容未变时以无响应体的 304 结束（[插件产物重校验](../bug-fix/2026-08-25-plugin-bundle-revalidation-etag.zh.md)）。外部脚本的 `error` 事件不给响应状态与正文，因此失败诊断只报告 URL；同源 host 供给与构建期写入的 registration id 是身份边界，`load` 后的工厂存在性检查负责拒绝未登记预期 id 的产物。
