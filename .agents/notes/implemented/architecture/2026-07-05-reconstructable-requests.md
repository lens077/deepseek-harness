...
- An absent referenced image object becomes deterministic placeholder text before dispatch ([missing image objects degrade to text](../bug-fix/2026-09-18-missing-image-objects-degrade-to-text.md)); the substitution is a function of the log and current object presence, not of any adapter. Corrupt or unreadable objects still fail model requests; [automatic attachment quarantine](../../proposed/bug-fix/2026-08-20-attachment-read-quarantine.md) records the proposed durable recovery without weakening byte-exact reconstruction.
