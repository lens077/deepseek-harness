...
The time/usage grouping and composer spacing remain in force. [Own-request usage governance](2026-09-12-own-request-usage-governance.md) owns the ledger-backed usage drawer and cost-first reading; the compact token-only dialog described here remains the fallback when that projection is absent.
