...
网络限制仍未决定：它可以作为独立的 `network_mode` 到来，也可以在某个 runner 同时强制两者后合并进 `sandbox_mode`。持久会话附加根目录已经属于 `SandboxPolicy`；[会话附加目录决策](2026-08-18-session-additional-directories.zh.md)将其与临时的逐调用升级授权区分开。
