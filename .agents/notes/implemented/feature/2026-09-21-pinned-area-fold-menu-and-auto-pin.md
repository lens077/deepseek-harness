...
**Durable `completed` as the unread rows only.** Rejected: acknowledging a reply marks it seen, but the digest's 已读未处理 section still holds finished work the user has viewed without handling. An unread-only set loses that work from the area after a restart. The handled mark, not the seen mark, is the user's "done with this"; the since-review window bounds the seen rows exactly as the digest does.
