...
**The question panel owns selection.** The Chat question panel gains a per-row trash entry (single) and a select mode with checkbox rows and one remove button (multi); both confirm inline inside the panel, because the panel closes on an outside pointer press and a confirmation outside it would be dismissed under the pointer. Rows whose turn is running, unloaded, already removed, or condensed cannot be picked, and the notice says why. After a removal the transcript keeps every row: the removed turn's rows render at reduced opacity through `data-context-removed` on the flow item, and a marker row shows where the checkpoint landed.
