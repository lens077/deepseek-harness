...
**Only the trailing gutter is reserved, and the leading one is the reader's.** The trailing padding takes `max(composer clearance + 16px, size + 32px)`, and a rail in its own column raises the reserve to a second control width. Nothing stands on the leading side, so it reserves nothing and became its own preference instead: `transcriptLeadingPad`, 0–60px in steps of five, default 5. Both pixel preferences share one `PixelPreferencePolicy` (field plus `PixelRange`) and one `PixelStepperRow`, so a second adjustable pixel value costs a descriptor rather than a copy of the policy and its row. The phone breakpoint drops the reserve entirely, where the column parks above the composer instead of beside the transcript. Measured live, the transcript's content box holds a constant 28px gap from the controls at 34px and at 50px.
