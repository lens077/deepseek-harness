...
`columns` is the board: [`selectColumns`](../../../../packages/client/ui-digest/src/client/select.ts) regroups the sections `selectInbox` produced into columns in decision order — waiting on the user, finished (unread and seen together), running, failed — each sorted by last activity newest first. Finished, running, and failed are present even when empty so their places hold; the waiting column appears only while the agent is waiting, because an empty slot for that uncommon state would take a quarter of the width from the three that carry the daily flow. It reads the sections rather than the rows so the window, workspace filter, handled visibility, and pin admission rules stay in one place; a pinned row keeps its state's column and a handled row joins finished or failed by outcome. The keyboard ring follows whichever arrangement is on screen. The board is desktop-only: phones list one column of rows anyway, so `mobileView` ignores the stored layout and the toggle is not offered there.

`showReply` off removes the reply and its truncation hint from the card and clamps the question to two lines; the head, changed files, and every action stay, so triage does not change, only density. The toggle is on the desktop toolbar and, on phones, inside the overview toolbar and the small layout's 更多操作 menu. The [package README](../../../../packages/client/ui-digest/README.md) owns the user-facing behavior.
...
**Give pinned rows their own column.** The board's value is a fixed place per state; a pinned column would take an equal-width slot from a state and move pinned rows out of the column that says what they need. The pin mark and `p` key still work in place.
...
The key legend is not shown on the board, where the columns fill the panel's height and scroll independently; the sections keep it. Below a 920px center column the board falls back to two columns and the panel body scrolls as a whole. Nothing model-facing changes.
