...
Network restriction remains open: it may arrive as a separate `network_mode` or merge into `sandbox_mode` once a runner enforces both. Durable additional session roots are part of `SandboxPolicy`; the [session additional-directories decision](2026-08-18-session-additional-directories.md) keeps them distinct from ad-hoc per-call escalation grants.
