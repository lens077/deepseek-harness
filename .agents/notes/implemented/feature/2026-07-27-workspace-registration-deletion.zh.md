...
菜单、`Modal` 和按钮保留现有结构与设计 token。永久删除 Session 仍是单独操作，由[不可逆的 Session 删除决策](2026-09-01-irreversible-session-deletion.zh.md)规定。

## 考虑过的替代方案

**级联删除会话。** 不予采纳，因为 Workspace 注册记录不拥有 Session 持久化，且产品需求是将历史记录保留在 Ungrouped 下。独立的[不可逆 Session 删除决策](2026-09-01-irreversible-session-deletion.zh.md)负责删除生命周期、实时状态检查、后代语义和明确 UI。
