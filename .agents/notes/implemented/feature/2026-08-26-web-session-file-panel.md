...
The second cut extends the panel to **session-tree changes** — the union of this session's changes and those of every descendant subagent session. Subagent work is invisible to the local derivation by design: a child works in its own session and the parent log records only the delegation tool call and result ([tool-subagent](../../../../packages/subagent/tool-subagent/README.md)). The extension reads [`subagents.list`](../../../../packages/subagent/subagent/src/index.ts), opens each child with [`session.follow`](../../../../packages/api/session-controller/src/index.ts), and reads older records with `session.page`. These operations serve live and cold children alike; the walk recurses through `hasChildren` to the whole tree and keeps one row per file with each segment labelled by its source (`reviewer · Turn 3 · edit`).
