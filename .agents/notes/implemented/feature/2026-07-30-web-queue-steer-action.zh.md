...
Composer 对新输入采用另一套尽力而为约定。所寻址会话空闲时，每个发送手势都执行普通 Queue 发送。主会话运行期间，Host 支持的 `busyEnter` 偏好选择 Queue（默认值）或 Steer；独立的[发送快捷键偏好](2026-09-06-configurable-composer-send-shortcut.zh.md) 将键盘手势映射到首选或另一种投递方式。已寻址 subagent 会让每个发送手势都使用其仅支持 Queue 的继续执行传输。Host settings 文档会在共享同一 DSH home 的 Web origin 之间持久化这些偏好；投递方式选择只影响支持 steering 的繁忙会话。如果 composer 直接发出的 Steer 错过当前 next-step 窗口，AgentLoop 会自动将其接纳为下一条唤醒 Queue 轮次，Web 不显示失败。
...
现有 `session.prompt(mode: 'steer')` 对主会话新输入仍采用尽力而为的约定：在 next-step 窗口之外，它会变为唤醒 agent 的后续轮次。Composer 会让显式 `queue | steer` 模式经过 slash 裁决与引用序列化，再调用该约定。浏览器提交策略拥有实时繁忙态 Enter 偏好，而 Host settings 服务拥有持久性；该策略只在 `enter` 模式下为支持 steering 的会话把普通 Enter 与加速 Enter 解析为互补手势，Settings 行和 InputBar 共享该策略，不重复实现存储或投递窗口权威。只有 Queue 行操作采用严格语义，因为任一种负面结果都会经原 Queue 单次入队项收敛。
