...
A chord may carry zero modifiers. This is what separates it from the send shortcut, which requires Ctrl, Meta, or Alt because it lives inside the composer: the toggle listener is on the document, so a bare key or a Shift chord is admitted and then kept silent inside editable fields by the same test the panel's triage keys use, while a custom chord with Ctrl, Meta, or Alt other than the reserved tab chords stays live everywhere, which is the reach-the-inbox-mid-sentence behavior the fixed chord had. The settings page states which of the two the current chord is.
...
Stored documents without the field read as `Ctrl+1`. Fixed `Ctrl+1`–`Ctrl+4` navigation takes precedence over the toggle and opens Inbox, Todos, Project todos, or Timeline without closing an already selected tab; editable fields, menus, and modal dialogs keep those keys. The desktop tabs carry visible keycaps, contrasting selected fills, and underlines. The panel legend names tab navigation, while the sidebar tooltip distinguishes the default Inbox entry from a custom toggle chord. A modifierless chord is unreachable while typing, by design; the page says so. As with the send shortcut, the OS or browser may still intercept an accepted chord.
