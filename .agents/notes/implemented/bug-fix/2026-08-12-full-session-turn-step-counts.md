...
The 2026-08-24 observability increment preserves that seam and expands the value with `turnMs`, `stepMs`, `toolCalls`, `toolResults`, `toolErrors`, `llmRetries`, `retryDelayMs`, and the six core `turn/end` outcome counters (`completedTurns`, `errorTurns`, `abortedTurns`, `blockedTurns`, `maxTokenTurns`, `interruptedTurns`). Complete turn/step lifecycle time now remains measurable when no assistant message assembles; model time retains its narrower assembled-message definition. The Web strip displays total turn time plus nonzero retry, tool-error, run-error, interruption, blocked, and token-limit signals. All additions are scalar log-derived metadata: they introduce no prompt, message, tool-payload, or error-text projection.
