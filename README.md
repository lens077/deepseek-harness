...
**Search on top, and a way to load everything under it.** The question rail used to put search last, and the loaded window is only a suffix of the session — a 60-turn conversation opens on its last 25 or so questions — so a filtered list was never the whole answer and the way to change that was buried inside the panel. The rail now reads top to bottom as search, load all history, previous, next. The load-all entry stands on the rail exactly while earlier pages remain, whether or not the panel is open; one press drives the transcript's own anchored paging until the session has no earlier events, keeping the reader's row in place, and the entry then leaves the rail. The panel hangs from the rail's top edge, level with the search entry that opened it, and is capped at the composer so a long list scrolls instead of running off the top of the viewport. Below, a 60-turn session opened on its tail page: the transcript still offers **加载更早** at its head, and the rail shows the four entries.

![A 60-turn session opened on its tail page: the transcript's load-earlier button at the head, and the rail reading search, load all, previous, next](docs/user/guide/question-rail-load-all.png)

After one press of load all: the load-earlier button and the rail entry are gone, and the question panel lists all sixty questions from the first, with no partial-window notice.

![The same session after load all: the panel lists every question from number 1, and the rail has only search and the two arrows](docs/user/guide/question-rail-loaded.png)
...
**The agent-preset label reads in the composer, and the title gets the header.** The read-only label naming the preset a session runs — **标准模式** by default — sat in the session header right after the title, and the title itself was capped at 220px, so a title longer than about fourteen Chinese characters ellipsized even on a wide window while the band beside it held one chip and empty space. The label now sits in the composer tool row, immediately left of the model select, sized like that chip; the header groups navigation facts (lineage, tabs, the session log) and the tool row groups run facts (access mode, preset, model), so each fact is where its kind lives. The current title takes whatever width the header row has and ellipsizes only when the row is narrower than its full text — a CSS rule, no measurement. On a blank session the label stays hidden: the new-session screen's chip already names the staged preset and is the one place to change it. Below, a session opened cold: a 29-character title shown whole, and the preset beside the model.

![An opened session with a 29-character title shown whole in the header, and the preset label reading 标准模式 in the composer tool row immediately left of the model select](docs/user/guide/preset-label-composer.png)

The new-session screen keeps the chip beside the workspace picker and shows no second copy in the composer:

![The new-session screen: the workspace picker, the start-without-a-folder action, and the 标准模式 chip above the composer, whose tool row holds only the access mode and the model](docs/user/guide/preset-label-hero.png)

**How the screenshots are made.** Every picture above is a real screen from this fork's own build, never a mock-up. The one exception to the isolation below is the first inbox picture, which is the author's own instance on a working day with session names and text blurred, because a fixture-seeded server cannot show nine workspaces of real traffic. To keep personal sessions and keys out of the rest, a second `dsh web` is started from the checkout's source against an isolated home (`DSH_HOME=/private/tmp/dsh-demo-home`, port 3099): its profile lists only the base bundle, the web app, and the vision bundle, and its `settings.yaml` declares a fictitious `acme-gateway` provider with an `acme-vision-think` model — no API key, no sessions, no workspace. The inbox pictures come from the same kind of isolated server, this time launched through the repository's own web test scaffold: six sessions are seeded from the e2e fixture logs under `apps/web/tests/snapshots/`, parsed with the repository's `parseSessionLog` and persisted through the real JSONL backend, two of them with their final `turn/end` rewritten to `interrupted` and `max-tokens` so the failed section has rows; Playwright at device scale 2 in `zh-CN` then adds a todo from a card, pins a row, switches tabs, and right-clicks a session row, and every picture is the screen at that moment. The question-rail pictures use the same scaffold with the session the load-all e2e generates — sixty closed turns of one short question and one short reply, three history pages, no model output anywhere in it — opened in Chromium at 1680×1000 and device scale 2 in `zh-CN`; the first picture is the tail page as it opens, the second is the screen after the rail's load-all entry was clicked and the search panel opened. The two preset-label pictures use the same scaffold once more: one session is seeded with a long Chinese title, one user message, and one text reply carrying a fictitious provenance, recorded under the shipped `standard` preset, and opened in Chromium at 1680×1000 and device scale 2 in `zh-CN` after connecting a temporary workspace; the hero picture is that screen before the seeded session is opened. Playwright drives Firefox at device scale 2 in the light theme, once per language, and clicks through the same path a user would: Settings → Models → Edit → Customized settings → Advanced. The Models-page pictures were then verified as a round trip rather than a rendering: checking two more levels and saving through the card rewrote the isolated `settings.yaml` with `off: null` and `minimal: minimal`, and the server's `llm.models` reply listed all seven levels for that model. The fixture provider and the fork's own product chrome are the only content, so nothing needs masking; the demo server, browser sessions, home directory, and temporary files are removed afterwards, and the personal `~/.dsh` and port 3080 are never touched. Any regression in the pictured behavior shows up in `pnpm run test:gui` (the component and shell suites) and in `DSH_SNAPSHOT=replay pnpm run test:web` (the assembled browser).
