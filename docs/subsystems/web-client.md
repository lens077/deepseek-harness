...
Navigation does not acknowledge results. The [Session Controller's Client completion reminders](../../packages/api/session-controller/README.md#client-completion-reminders) include the selected Session and clear through an explicit Client-only operation. Host status events atomically carry a complete projection baseline; matching Host and Client definitions require it, and the Client applies higher-seq-wins before evaluating reminders rather than relying on control-stream arrival order. [Chat](../../packages/client/ui-chat/README.md#reply-exposure) owns rendered foreground-answer exposure; [ui-digest](../../packages/client/ui-digest/README.md#reading-and-acknowledgement) owns the reading interval, manual acknowledgement, durable seen writes, and reconciliation of local reminders. Viewed, handled, and pinned remain separate facts. DOM observation and reading timers stay outside the shared Session model, and these presentation facts add no model-visible Session events.
