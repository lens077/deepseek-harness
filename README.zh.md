...
**用持久的收件箱取代绿点。** 跨几个工作区跑了一夜的任务，第二天早上没有任何记录：侧边栏的"已完成"小圆点只是浏览器内存里的一个位，点一下或刷新就没了，卡在审批上的会话也无人察觉。「新会话」下方的 **汇总** 入口现在携带需要你处理的会话数，并在中间栏打开一个收件箱。行按需要关注的原因分区——等你回复、失败或中断、完成但未读、已读未处理、运行中——时间窗默认为"自上次查看"，工作区 chip 带各自的计数，卡片提供打开、带预填输入框的继续、标记已处理、加入待办、置顶、明天再看。`Ctrl+1`（或在 **汇总面板** 设置页录制的任意按键或组合键）可从任何位置开关面板，在输入框里打到一半也能用；卡片操作则在键盘环上（`j`/`k`、`Enter`、`e`、`t`、`p`、`s`）；"复制晨报"把各分区以 Markdown 放进剪贴板。每个标记都存在 Host 上——看过哪条回复、处理了什么、延后或置顶了什么——因此刷新、换浏览器、重启都还在，这个数字也会前缀到浏览器标签页标题。卡片永远不会高过面板：头部与操作行固定不动，长回答在卡片内部滚动；**打开会话** 会在侧边栏揭示该会话——重新展开折叠的工作区、撤销**显示更多**截断、展开嵌套分支、并把选区移到该行——因此高亮的行始终就是你打开的那一个。第四个标签 **项目待办** 列出配置的根目录与每个工作区里找到的 `TODO.md` 类文档。

![覆盖中间栏的收件箱：失败/中断分区、带计数的工作区 chip、在卡片内部滚动的长回答与固定在下方的操作行，以及待办、项目待办、时间线标签](docs/user/guide/inbox-panel.png)
...
**搜索在最上，加载全部紧随其下。** 提问轨道此前把搜索放在最后，而已加载的窗口只是会话的一个后缀——60 轮的对话打开时只有最后 25 问左右——因此过滤出的列表从来不是完整答案，而改变这一点的办法又埋在面板里面。轨道现在从上到下依次是搜索、加载全部历史、上一个、下一个。加载全部入口恰好在还有更早分页未载入时常驻轨道，无论面板是否打开；按一次即沿转录自己的带锚定分页逐页回翻，直到会话没有更早事件，期间读者所在行保持不动，之后该入口离开轨道。面板从轨道顶端垂下，与打开它的搜索入口齐平，高度封顶在输入框之上，长列表在面板内滚动而不是冲出视口顶部。下图是一个 60 轮的会话在尾页打开的样子：转录头部仍有**加载更早**，轨道上是四个入口。

![一个 60 轮会话在尾页打开：转录头部的加载更早按钮，轨道依次为搜索、加载全部、上一个、下一个](docs/user/guide/question-rail-load-all.png)

按一次加载全部之后：加载更早按钮和轨道入口都消失了，提问面板从第 1 问起列出全部 60 问，没有窗口不完整提示。

![同一会话加载全部之后：面板从第 1 问起列出每一问，轨道只剩搜索与上下两个箭头](docs/user/guide/question-rail-loaded.png)
...
**会话连选与多选。** 工作区的会话行此前没有自己的选择概念：单击即「打开」，唯一高亮的行是当前打开的会话。现在会话行支持 Windows 资源管理器式的选择，默认开启，可在「通用」设置里关闭。`Ctrl`+单击（macOS 为 `Cmd`，因为该平台上 `Ctrl`+单击是系统次级点击）切换单行，`Shift`+单击取从锚点开始的闭区间，`Ctrl`+`Shift`+单击把新区间并入已有选区；所有带修饰键的手势都不会打开会话，也不会启动重排拖拽。列表获得焦点后，`Ctrl`/`Cmd`+`A` 选中全部可见行，`Esc` 清空，方向键移动 lead，`Shift`+方向键扩展。区间基于实际渲染的行计算，因此可以跨工作区分组，但不会触及折叠的分组或 **展开其余** 截断之后的行。选区是浏览器本地状态且有意不做持久化，因为视图 store 按整值持久化，恢复出来的选区会高亮没人选过的行。设计与被否决的方案记录在 [多选 Agent Note](.agents/notes/implemented/architecture/2026-08-31-workspace-session-multi-selection.zh.md)。

![四个会话行作为连续区间跨两个工作区分组被选中，最后一行带有键盘 lead 的描边](docs/user/guide/workspace-session-multi-select.png)
...
**preset 标签移到输入框，标题栏留给标题。** 说明会话运行哪个 preset 的只读标签——默认是**标准模式**——原本紧跟在会话标题之后，而标题本身被限制在 220px，于是十四个汉字左右的标题在宽窗口下也会被省略，旁边却只有一枚 chip 和一片空白。现在标签落在输入框工具行、模型选择器紧左侧，尺寸与那枚 chip 一致；标题栏聚集的是导航事实（谱系、标签页、会话日志），工具行聚集的是运行事实（访问模式、preset、模型），每项事实都在它的同类里。当前标题占用标题栏整行可用的宽度，只有整行比全文还窄时才省略——一条 CSS 规则，不做任何测量。会话为空时标签保持隐藏：新建会话界面上的 chip 已经写明暂存的 preset，也是唯一可以修改它的地方。下图是冷打开的一个会话：29 个字的标题完整显示，preset 在模型旁边。

![打开的会话：29 字的标题在标题栏完整显示，输入框工具行里模型选择器紧左侧是"标准模式"标签](docs/user/guide/preset-label-composer.png)

新建会话界面仍把 chip 放在工作区选择器旁边，输入框里不再出现第二份：

![新建会话界面：工作区选择器、"不选目录，直接开始"与"标准模式"chip 位于输入框上方，工具行里只有访问模式和模型](docs/user/guide/preset-label-hero.png)

**截图是怎么拍的。** 上面每一张图都是这个 fork 自己构建出来的真实界面，不是效果图。下述隔离方式唯一的例外是收件箱的第一张图：那是作者自己的实例在某个工作日的画面，会话名与正文已打码，因为用 fixture 播种的服务拍不出九个工作区的真实流量。其余的图为了不把个人会话和密钥拍进去，会从 checkout 的源码另起一个 `dsh web`，指向一个隔离的家目录（`DSH_HOME=/private/tmp/dsh-demo-home`，3099 端口）：它的 profile 只列出基础 bundle、Web 应用和视觉 bundle，`settings.yaml` 只声明一个虚构的 `acme-gateway` 提供方和一个 `acme-vision-think` 模型——没有 API 密钥、没有会话、没有工作区。收件箱的几张图来自同类的隔离服务，这次通过仓库自己的 Web 测试脚手架启动：用 `apps/web/tests/snapshots/` 下的 e2e fixture 日志播种六个会话，经仓库的 `parseSessionLog` 解析后通过真实的 JSONL 后端持久化，其中两个把最后的 `turn/end` 改写为 `interrupted` 与 `max-tokens`，以便失败分区有内容；随后 Playwright 以设备缩放 2、`zh-CN` 从卡片加入一条待办、置顶一行、切换标签、右键一个会话行，每张图都是当时的真实画面。提问轨道的两张图用同一套脚手架，会话是加载全部 e2e 生成的那种——60 个已结束的回合，每回合一句短提问和一句短回复，三页历史，其中没有任何模型输出——以 Chromium 在 1680×1000、设备缩放 2、`zh-CN` 下打开；第一张是尾页刚打开的画面，第二张是点击轨道上的加载全部入口并打开搜索面板之后的画面。preset 标签的两张图再次使用同一套脚手架：播种一个会话，带一条较长的中文标题、一条用户消息和一条标注了虚构来源的文本回复，记录在随附的 `standard` preset 之下，连接一个临时工作区后以 Chromium 在 1680×1000、设备缩放 2、`zh-CN` 下打开；新建会话那张是打开播种会话之前的画面。Playwright 以设备缩放 2、浅色主题驱动 Firefox，每种语言各跑一遍，并沿用户真实的路径点击：设置 → 模型 → 编辑 → 自定义设置 → 高级。模型页的图随后按一次往返而非一次渲染来核验：再勾选两个档位并通过卡片保存，隔离的 `settings.yaml` 被改写为带 `off: null` 与 `minimal: minimal`，服务端 `llm.models` 的回复也为该模型列出了全部七个档位。画面里只有虚构提供方和 fork 自己的产品外壳，因此无需打码；演示服务、浏览器会话、家目录和临时文件事后全部删除，个人的 `~/.dsh` 与 3080 端口自始至终不被触碰。图中行为一旦回退，会在 `pnpm run test:gui`（组件与外壳测试）和 `DSH_SNAPSHOT=replay pnpm run test:web`（组装后的浏览器）中暴露。
