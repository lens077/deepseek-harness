...
describe('sandbox escalation API (write/edit)', () => {
  ...
  it('a plain write stamps the default mode with the calling session root', async () => {
    ...
    expect(fs.stamped).toEqual([{ mode: 'workspace-write', workspaceRoots: [resolve('/session-project')] }])
  })
  ...
  it('a standing session override folds onto the stamp', async () => {
    ...
    expect(fs.stamped).toEqual([{ mode: 'read-only', workspaceRoots: [resolve('/session-project')] }])
  })
  ...
  it('an approved escalation stamps the granted mode onto that write', async () => {
    ...
    expect(fs.stamped).toEqual([{ mode: 'danger-full-access', workspaceRoots: [resolve('/session-project')] }])
  })
