...
`session.searchQuestions` 是其会话内对应物：它搜索单个会话的完整日志中当前的用户提问，而非客户端已加载的事件窗口，并返回每个命中的 `seq`、`time` 和有界 snippet——绝不包含消息正文、工具调用或图片。网关以与 `session.history` 相同的方式读取目标会话，因此调用方无权读取的 id 会在任何查询到达索引之前以 `session-not-found` 失败；随后向 `ctx.sessionQuery.searchEvents` 请求至多 50 个当前 `user/message` 命中，并丢弃提供方返回的任何不属于该会话、内容视图或类型的命中。当提供方仍持有续传游标时，`complete` 为 false；只有在收到完整的空页面时，客户端才可以报告没有匹配的提问。索引未挂载、会话不可读和查询被中止分别是可区分的业务错误，因此客户端绝不会把其中任何一种渲染成空结果。
