...
export const rpcErrorSchema: z.ZodType<RpcError> = z.discriminatedUnion('code', [
  ...
  z.object({ code: z.literal('workspace-membership-invalid'), message: z.string(), details: z.object({ workspaceId: z.string(), sessionIds: z.array(z.string()), member: z.boolean() }) }),
  ...
  z.object({
    code: z.literal('directory-invalid'),
    message: z.string(),
    details: z.object({
      path: z.string(),
      reason: z.union([z.literal('not-absolute'), z.literal('unavailable'), z.literal('not-directory')]),
    }),
  }),
