...
describe('AppFrame phone presentation', () => {
  it('applies phone density and font only below the mobile breakpoint', () => {
    ...
    // The phone sidebar spans the frame and receives the navigation share.
    expect(sidebarOwner().width).toBe(390)
    expect(typeof (sidebarOwner() as { navigateMobile?: unknown }).navigateMobile).toBe('function')
    // No drag handles on a phone: the columns are not resizable surfaces.
  ...
  it('switches the phone surface to the conversation when another Session is selected', () => {
    selectedSession = 'first' as SessionId
    frameWidth = 390
    const { frame, rerenderFrame, sidebarOwner } = mountFrame(390)
    expect(frame.dataset.mobileView).toBe('overview')
    act(() => { (sidebarOwner() as { navigateMobile: (view: string) => void }).navigateMobile('workspaces') })
    expect(frame.dataset.mobileView).toBe('workspaces')
    // The conversation layer marks itself inert while a phone surface covers it.
    expect(frame.querySelector('[aria-hidden="true"]')).not.toBeNull()
    selectedSession = 'second' as SessionId
    rerenderFrame()
    expect(frame.dataset.mobileView).toBe('conversation')
    expect(frame.querySelector('[aria-hidden="true"]')).toBeNull()
    // A pending new-Session surface keeps its place across the switch.
    act(() => { (sidebarOwner() as { navigateMobile: (view: string) => void }).navigateMobile('new') })
    selectedSession = 'third' as SessionId
    rerenderFrame()
    expect(frame.dataset.mobileView).toBe('new')
  })
})
