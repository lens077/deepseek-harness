...
describe('phone navigation sizing', () => {
  it('reserves the bottom safe area only when the presentation owns system insets', () => {
    ...
    expect(frame).toMatch(
      /@media \(display-mode: standalone\), \(display-mode: fullscreen\)\s*\{\s*\.frame\[data-mobile\]\s*\{\s*--mobile-nav-safe-area: env\(safe-area-inset-bottom, 0px\)/s,
    )
    ...
    expect(sidebar).toMatch(
      /\.navigation\s*\{[^}]*height: var\(--mobile-nav-height\);[^}]*padding-bottom: var\(--mobile-nav-safe-area\);[^}]*box-sizing: border-box/s,
    )
