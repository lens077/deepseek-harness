...
桌面端持久化的 `desktopLayout` 密度会调整侧栏、会话顶部栏、标签和输入区周围的间距，但不会替换三栏结构。宽度小于 768px 时，框架在顶部栏与固定底部导航之间显示一个全宽手机界面，并隐藏两侧拖动手柄。框架使用 `100dvh`；独立应用和全屏显示模式加入底部安全区域 inset，普通浏览器模式不额外加入 inset。框架持有临时的手机导航状态，并将其传给侧栏与 `center.overlay` 条目；桌面端面板几何状态保持独立。右栏在手机上不保留轨道：右侧 Sidebar 在同一断点以下自行切换为全屏呈现，并相对于视口定位。
