...
在 root 槽位挂载本插件；它随即围绕占据侧栏、会话与详情栏的内容渲染应用框架。用户拖动不可见命中条带缩放侧栏、拖动浮动胶囊缩放详情面板；窗口变窄时只有详情栏收缩，随后自动关闭。桌面端关闭的侧栏保留 56px 控制栏；详情栏关闭到零宽度。

宽度小于 768px 时，框架在顶部栏与固定底部导航之间显示一个全宽手机界面，并隐藏拖动手柄。框架使用 `100dvh`；独立应用和全屏显示模式加入底部安全区域 inset，普通浏览器模式不额外加入 inset。框架持有临时的手机导航状态，并将其传给侧栏与中央覆盖层；桌面端面板几何状态保持独立。
