...
agent preset 的各个表层：General 设置中的一行，用于选择新建会话据以组装的 [preset](../../preset/agent-presets/README.zh.md)；新建会话界面上的一枚 chip，用于选择**下一个会话**的 preset；composer 工具行中的一个只读标签；以及一个设置页分区，用于管理名单——复制、删除、默认值，以及通往 preset 自身文件的入口。
...
第三个表层，位于 composer 工具行、模型选择器的左侧（`conversation.input.right`）：**本会话**所运行的 preset，作为静态装饰呈现，尺寸与旁边的模型 chip 一致。在那里放一个控件，等于承诺一次宿主会断然拒绝的切换。它落在 composer 而非会话标题旁，是为了把标题栏的宽度留给标题本身；会话为空时它保持隐藏，此时新建会话 chip 已经写明暂存的选择，也正是修改它的地方。它从会话自身的摘要读取 preset，并在 General 行所读的同一份名单上解析显示名称。转发的 owner 事件 `agent-preset/selected` 会在每个标签页中把已经提交的空会话切换折进这份共享摘要；发起方标签页可能已经采用 RPC 回执，而合并是幂等的。
