...
同一个展开区还编辑 pi-ai 行的两项能力声明 `input` 与 `reasoningEfforts`：手动录入的模型在自行声明之前是纯文本、不推理的，而没有任何环节能去询问端点。**图片输入**提供跟随目录默认（字段缺席）、文本 + 图片、仅文本三个选项；下拉框无法表达的手写列表会如实显示、绝不改写，直到被另一个选项替换。**推理**提供跟随目录默认（缺席）、不推理（`false`）与自定义可选档位：以勾选档位为键的 pi-ai 七档字典，新勾选的档位按规范名称写入（`high: high`，`off` 不带值），已有的名称则保留。切换到自定义档位时默认勾选 `low`、`medium`、`high`。行检查器会拒绝 `text`/`image` 以外的模态、既非 `false` 也非字典的推理值、适配器不认识的档位、`off` 以外却没有名称的档位，以及只剩 `off` 的字典——与适配器加载时的拒绝一致，只是按行点名。`compat` 与改写的发送名称仍留在 `settings.yaml` 中。
...
- **卡片上可编辑的只有 API 密钥与精选折叠区字段**：手写编辑器用 schema 通用的字段覆盖面换来了设计稿上的布局（[Agent Note](../../../.agents/notes/implemented/architecture/2026-07-30-web-config-plane.zh.md)）。两个家族都公开 `baseURL` 与模型的 `id`/`name`/`contextWindow`/`maxTokens`；pi-ai 行还公开 `input` 与 `reasoningEfforts`，手工声明的 pi-ai 路由还公开 `displayName` 与 `api`。重试策略、超时、逐模型的 `compat`、改写的推理发送名称、DeepSeek 模型说明及其他进阶字段仍留在 `settings.yaml` 中；编辑器未展示的现有模型字段会予以保留。不带这些约定字段的 profile schema 只渲染该提示，两套精选布局则以 `llm-deepseek`/`llm-pi-ai` 这两个 namespace 的名字为键。
