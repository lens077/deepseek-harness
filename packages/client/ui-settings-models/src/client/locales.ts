...
export const en = {
  ...
  modelInput: 'Image input',
  modelInputInherit: 'Catalog default',
  modelInputMultimodal: 'Text and images',
  modelInputText: 'Text only',
  modelInputCustom: 'As written in settings.yaml',
  modelReasoning: 'Reasoning',
  modelReasoningInherit: 'Catalog default',
  modelReasoningNone: 'Non-reasoning model',
  modelReasoningLevels: 'Selectable levels',
  modelReasoningLevelsHint: 'Each checked level is offered in the model selector and sent under its own name. A hand-entered model is text-only and non-reasoning until these say otherwise; rename a level\'s wire spelling or add compat switches in settings.yaml.',
  modelInputInvalid: 'Image input must be a list made of text and image.',
  modelReasoningInvalid: 'Reasoning must offer at least one level other than off, each level naming its wire spelling.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  modelInput: '图片输入',
  modelInputInherit: '跟随目录默认',
  modelInputMultimodal: '文本 + 图片',
  modelInputText: '仅文本',
  modelInputCustom: '按 settings.yaml 所写',
  modelReasoning: '推理',
  modelReasoningInherit: '跟随目录默认',
  modelReasoningNone: '不推理',
  modelReasoningLevels: '自定义可选档位',
  modelReasoningLevelsHint: '勾选的档位会出现在模型选择器中，并按原名发送。手动添加的模型在此处声明前视为仅文本、不推理；改写档位的发送名称或添加 compat 开关请编辑 settings.yaml。',
  modelInputInvalid: '图片输入必须是由 text、image 组成的列表。',
  modelReasoningInvalid: '推理至少要提供一个 off 以外的档位，且每个档位都要有发送名称。',
  fetchModels: '获取可用模型',
  fetching: '正在询问提供方\u2026',
