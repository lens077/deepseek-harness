...
client entry 还持有 slot outlet、会话 provider 以及 observable 到 uSES 绑定的 React 实现。业务插件通过带类型的 slot `hooks` 传递裸 observable source；渲染器在 outlet 处完成绑定。插件在 `slots`、`sessions` 和 `layout` 就绪后激活，投影当前会话标题，并执行全程序唯一一次上下文级 `renderSlot('root')` 调用。浏览器标题还携带环境运行标记，以及位于其前的、上报到本插件提供的 `documentBadge` 座位的计数（`(N)`；为零时不显示），因此知道有多少事情需要用户的插件（[ui-digest](../ui-digest/README.zh.md)）无需拥有标题就能在标签页上说明。React、React DOM、Cordis、ui-slots 和 ui-primitives 通过 web 外壳的静态模块表保持同一浏览器身份；本包则以动态客户端 bundle 到达。
