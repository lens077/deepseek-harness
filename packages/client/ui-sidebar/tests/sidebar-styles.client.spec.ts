...
describe('SidebarRoot.module.css', () => {
  it('shares and cancels the wide shell trailing padding structurally', () => {
    ...
    expect(root?.get('--dsh-sidebar-inline-padding')).toBe('var(--dsh-desktop-sidebar-inline-padding, 12px)')
    expect(root?.get('padding')).toBe(
      'var(--dsh-desktop-sidebar-block-padding, 6px) var(--dsh-sidebar-inline-padding)',
    )
  ...
  it('stacks the reduced brand row over the smaller fork tagline', () => {
    expect(declarations('.brandIdentity')?.get('flex-direction')).toBe('column')
    expect(declarations('.brandRow')?.get('height')).toBe('20px')
    ...
    expect(declarations('.brandName svg')?.get('height')).toBe('20px')
    expect(declarations('.brandName svg')?.get('width')).toBe('auto')
    ...
    expect(declarations('.brandTagline')?.get('font-size')).toBe('10px')
    expect(declarations('.brandTagline')?.get('white-space')).toBe('nowrap')
    expect(declarations('.brandTagline')?.get('text-overflow')).toBe('ellipsis')
  })
})
