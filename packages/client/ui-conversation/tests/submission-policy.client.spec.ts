...
describe('ComposerSubmissionPolicy', () => {
  ...
  it('adopts a Host preference without writing it back and leaves an identical write untouched', () => {
    ...
    host.publish({ status: 'ready', value: { busyEnter: 'steer', questionNavigation: { previousShortcut: 'Ctrl+ArrowUp', nextShortcut: 'Ctrl+ArrowDown', focusPolicy: 'editable', expandButtonSide: 'right' } }, revision: 1, writable: true })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
    policy.setBusyEnter('steer')
    expect(host.set).not.toHaveBeenCalled()
    host.publish({ value: { busyEnter: 'steer', questionNavigation: { previousShortcut: 'Ctrl+ArrowUp', nextShortcut: 'Ctrl+ArrowDown', focusPolicy: 'editable', expandButtonSide: 'right' } }, revision: 2 })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
  })
