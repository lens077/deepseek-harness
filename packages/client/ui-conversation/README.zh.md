...
设置 > 通用提供 `sendShortcut` 选项：`enter`（默认值）、`mod-enter`（Ctrl/Cmd+Enter）、Alt+Enter、Ctrl+Shift+Enter、Cmd+Shift+Enter，或自定义组合键。选择自定义，按下组合键，然后保存；Escape 或取消会丢弃录制结果，重置则恢复 Enter。composer 工具条在模型座位旁提供承载同一偏好的选择芯片，因此任何会话都可以不打开设置就切换预设；已录制的自定义组合键在其中单独成行，录制新组合键仍在设置中进行。两处写入同一份持久偏好，因此在任一处修改都会立即在各处生效。显式组合键必须包含 Ctrl、Meta（显示为 Cmd）或 Alt，加上一个受支持的按键，也可包含 Shift。支持字母、数字、Enter、Space、方向键、Home、End、PageUp/PageDown 和 F1–F12，但保留的编辑与浏览器快捷键除外。Ctrl 与 Meta 不等价；修饰键必须完全匹配。操作系统或浏览器可能在 composer 收到按键前拦截其他组合键。

主会话运行时，`busyEnter` 选择首选的 Queue 或 Steer 投递方式：`enter` 用 Enter 执行首选方式、Ctrl/Cmd+Enter 执行另一种方式；`mod-enter` 用 Ctrl/Cmd+Enter 执行首选方式、Enter 换行。显式组合键只在完全匹配时发送，并使用首选投递方式；普通 Enter 与不带其他修饰键的 Shift+Enter 用于换行。IME 输入组合期间绝不提交。空草稿 Ctrl/Cmd+Enter 全部插话手势仅在 `enter` 或 `mod-enter` 下可用；自定义快捷键不会在草稿为空时将队列插话发送。指针操作独立于快捷键（[决策](../../../.agents/notes/implemented/feature/2026-09-06-configurable-composer-send-shortcut.zh.md)）。

设置 > 通用还提供 `homeEndInTextFields`。在 composer 中，Home 与 End 始终把光标移到所在行的行首和行尾，按住 Ctrl/Cmd 则扩展到整份草稿，Shift 从固定端扩展选区；引用芯片占一个位置，因此行首行尾绝不会落在引用内部，Alt 组合留给工作区固定项快捷键。macOS 即使在文本框聚焦时也把 Home/End 绑定为滚动文档，因此 composer 直接依据 EditorState 按逻辑行计算移动，不依赖非标准的 `Selection.modify`。该偏好管辖 GUI 中其它 `<input>` 与 `<textarea>`：默认值 `true` 让它们使用同一手势，`false` 则交还浏览器。其监听器运行在冒泡阶段并跳过已被处理的按键，因此自行占用 Home/End 的输入框或面板保持原有行为，而焦点不在文本框时仍然滚动（[决策](../../../.agents/notes/implemented/feature/2026-09-24-home-end-caret-in-text-fields.zh.md)）。
