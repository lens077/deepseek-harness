...
- [Question navigation policy](#question-navigation-policy)
...
The resident composer survives no-Session and Session transitions. With a rail occupant, the shell preserves the same scrollport, composer, input, and Workspace-chip DOM ancestry when the first Session appears; only the rail panel's Session content is conditional. Adaptive width handles share the transcript's grid area, excluding the Files rail, so rail width does not shift their hit regions onto composer controls. The no-Session state keeps the same composer surface mounted but inert while the Workspace picker connects a blank Session. The surface is a shell-owned Lexical editor: reference chips are atomic decorator nodes carrying the owner's serialization identity (submission expands them through the owner codec), claimed slash commands stay styled leading text, folder text references carry the folder glyph as an icon prefix, and the draft's clipboard projection is mirrored into the per-Session Conversation store. Queue operations address exact queue occurrences through the scoped `ctx.conversation` service; queue previews render sent text through the shared inline reference projection from `ui-primitives` (wire session forms fold to their label) and show local or durable images and files in original attachment order. Images use thumbnails; files use compact name-and-size cards. An edit exposes the literal sent text, and durable thumbnails resolve through the session image URL cache. Composer keyboard preferences are stored in the Host-backed `ui-conversation` settings namespace. The same namespace stores the conversation content-width mode: the default `fill` keeps the transcript at 100% of the content area, while `adaptive` enables the draggable side width handles with the adaptive clamp.

A materialized scratch Session with a resolved cwd does not require Workspace membership to enable the composer; ordinary model and interaction blockers still apply. Below 768px, the conversation uses the available width and hides transcript resize handles. The shell's Files disclosure starts closed and reveals the resident rail over the conversation instead of reserving a side column.

Settings > General offers `sendShortcut`: `enter` (default), `mod-enter` (Ctrl/Cmd+Enter), Alt+Enter, Ctrl+Shift+Enter, Cmd+Shift+Enter, or a custom combination. Choose Custom, press the combination, then Save; Escape or Cancel discards the recording, and Reset restores Enter. The composer toolbar carries the same preference as a chip beside the model seat, so any conversation can switch presets without opening Settings; a recorded custom combination appears there as its own row, and recording a new one stays in Settings. Both surfaces write the one durable preference, so a change from either is immediately in effect everywhere. Explicit combinations require Ctrl, Meta (displayed as Cmd), or Alt plus a supported key, with optional Shift. Letters, digits, Enter, Space, arrows, Home, End, PageUp/PageDown, and F1–F12 are supported, except reserved editing/browser shortcuts. Ctrl and Meta are distinct; the exact modifiers must match. The OS or browser may intercept other combinations before the composer receives them.

For a running primary session, `busyEnter` selects the preferred Queue or Steer delivery: `enter` uses Enter for that preference and Ctrl/Cmd+Enter for the alternate; `mod-enter` uses Ctrl/Cmd+Enter for the preference and Enter for newlines. Explicit combinations send only on an exact match and use preferred delivery; ordinary Enter and bare Shift+Enter insert newlines. IME composition never submits. The empty-draft Ctrl/Cmd+Enter steer-all gesture is available only with `enter` or `mod-enter`; custom shortcuts do not steer an empty draft's queue. Pointer actions are independent of the shortcut ([decision](../../../.agents/notes/implemented/feature/2026-09-06-configurable-composer-send-shortcut.md)).

Settings > General also offers `homeEndInTextFields`. In the composer, Home and End always move the caret to the start and end of the caret's line, Ctrl/Cmd widens the reach to the whole draft, and Shift extends the selection from the fixed end; a chip counts as one position, so an edge never lands inside a reference, and Alt is left to the workspace pin shortcuts. macOS binds Home/End to document scrolling even while a text field holds focus, so the composer computes the motion from its EditorState over logical lines rather than relying on the non-standard `Selection.modify`. The preference governs the GUI's other `<input>` and `<textarea>` fields, where the default `true` applies the same gesture and `false` returns them to the browser. Its listener runs in the bubble phase and skips an already-handled press, so a field or panel that claims Home/End keeps it, and a focus outside a text field still scrolls ([decision](../../../.agents/notes/implemented/feature/2026-09-24-home-end-caret-in-text-fields.md)).

Default sends commit optimistically: submission clears the draft, occurrence table, and undo history in the same transaction, keeps the composer in `plain`, and runs the send as a detached attempt, so typing and further sends continue during the flight. `sendSession` registers a Session submission echo (`session.beginSubmission`) with the delivery mode before serializing, preserving selected image and file order in `pendingSubmissions`; Session derives the placement from that mode and its current running state, so idle sends use the transcript, busy Queue sends use QueueDock, and busy Steer sends use the pending-steering surface. It then yields one paint, encodes images through the browser's native `FileReader` data-URL path, and cites staged file receipts. Command submissions use the same receipts for generic files, so sending `/goal` or `/plan` never reads those browser files again. The prompt reuses the submission `requestId`; queue and history observation by that `rpcId` retires the echo once. Concurrent failures are restored together in submission order until the user edits the restored content; command submissions keep the frozen `submitting` phase. Detached attempts retain their attachment ids through admission and Session scope disposal. An observed retirement immediately exposes each image preview through the durable cache, replaces it with the canonical URL after fetching the admitted attachment, revokes each URL after its use ends, and releases file cards. Selected generic files enter one FIFO background-upload queue; `maxConcurrentFileUploads` defaults to two active Worker transports, the Conversation service retains queued and active operations plus byte progress across Session navigation, and removing a draft skips its queued transfer or aborts its active transport. Continuable subagents disable attachment intake and skip local echoes because their transport does not preserve the browser request id.
...
The Host-backed `ui-conversation` settings namespace stores the previous and next shortcuts, their focus policy, and the side carrying the question-bar expand control. `QuestionNavigationPolicy` publishes one reactive `questionNavigation` service; `ui-chat` consumes it for keyboard stepping and sticky-bar layout. Recording rejects a modifier alone and duplicate bindings, and an unmodified printable key requires explicit confirmation before it replaces a shortcut.
