...
Target packages declaration-merge their snapshot and Location data maps, then register with `ctx.uiConversation.events.register(...)` and `ctx.uiConversation.views.register(...)`. A target reads its Session-owned source with `ctx.uiConversation.binding(binding).target(targetId)`. Registrations are Cordis effects and their returned disposers remove the contribution from the same registry. An entry outside the shell, such as a composer dock strip, selects a View for a mounted Session with `ctx.uiConversation.openView(sessionId, view, focus?)`; the shell installs that opener when its Session body mounts and releases it with the Session binding, so an unmounted Session rejects the call.
...
Settings > General offers `sendShortcut`: `enter` (default), `mod-enter` (Ctrl/Cmd+Enter), Alt+Enter, Ctrl+Shift+Enter, Cmd+Shift+Enter, or a custom combination. Choose Custom, press the combination, then Save; Escape or Cancel discards the recording, and Reset restores Enter. Explicit combinations require Ctrl, Meta (displayed as Cmd), or Alt plus a supported key, with optional Shift. Letters, digits, Enter, Space, arrows, Home, End, PageUp/PageDown, and F1–F12 are supported, except reserved editing/browser shortcuts. Ctrl and Meta are distinct; the exact modifiers must match. The OS or browser may intercept other combinations before the composer receives them.
