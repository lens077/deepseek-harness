...
读取列表回答的是 agent 此刻在做什么，因此它保持按新近排序的平铺清单——每行是一条舍头留尾的完整路径——在 agent 运行期间保留最近二十条读取，空闲时为空：这是对 `running` 的推导，不是定时器，也不是一份存下来的清单。选中一个文件会调用 `revealFile`，滚动到最后一个以 `data-file` 携带该路径的工具行；在已加载窗口之外被修改的文件没有对应行，也就没有匹配。当快照报告 `hasMore` 时，侧栏声明该清单不完整并提供 `加载全部`，逐次分页拉取剩余历史。

子代理在自己的会话里工作，父会话日志只记录委派调用及其结果，因此委派出去的回合对本地推导没有贡献。`SessionTreeController` 直接读取后代来补上这个缺口：`subagents.list` 遍历持久化的子会话目录，`session.follow` 在固定 cursor 打开活跃或冷子会话，`session.page` 读取更早的记录。`deriveTreeFiles` 从这些记录中的原始调用与持久化结果 metadata 读取改动，因此与本地 Chat 快照使用同一套 mutation 词汇。打开面板时只读取第一层已完成的子会话且每个一页；`加载全部` 递归整棵树、逐页读取每个后代，并在同一个动作中补全本地历史。
...
宽度小于 768px 时，文件栏填满可用宽度并隐藏缩放手柄，不改变已存储的桌面宽度。[对话 shell](../ui-conversation/README.zh.md)负责手机端展开控件，在对话上方显示或隐藏文件栏。
