...
function changeNode(seq: number, path: string) {
  return {
    ...
    callTime: seq - 1, content: [], isError: false,
    meta: { diffs: [{ path, oldText: 'before', newText: 'after' }] },
    subCalls: [],
  }
}
...
function fakeSession(pages: number, nodes: readonly unknown[] = []) {
  ...
  return {
    ...
    chat: {
      ...EMPTY_CHAT_SNAPSHOT,
      timeline: { turnOrder: [1], turns: new Map([[1, { start: { seq: 0 } }]]) },
      legacy: { ...EMPTY_CHAT_SNAPSHOT.legacy, nodes: nodes as never },
    },
    loadOlder: vi.fn(() => {
      remaining--
...
async function bench(session: ReturnType<typeof fakeSession> | null = fakeSession(0), chat = true) {
  ...
  ctx.provide('uiConversation', {
    binding: () => ({ target: () => ({ getSnapshot: () => session?.chat }) }),
  } as never)
  ...
  const sessionRemote = {
    ...
    page: () => Promise.resolve({ ok: true as const, value: { records: [], hasMore: false } }),
  }
  ctx.provide('remote', { session: sessionRemote, subagents } as never)
...
describe('session-files browser plugin', () => {
  ...
  it('takes both seats and releases them with the fiber', async () => {
    ...
    expect(inject).toEqual([
      'sessions', 'uiConversation', 'slots', 'locale', 'settingsScope',
      'remote', 'remote.session', 'remote.subagents',
    ])
