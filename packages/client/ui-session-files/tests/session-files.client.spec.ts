...
function result({ seq, callView = null, resultView = null, isError = false, tool = 'edit' }: ResultOptions): ConversationNode {
  const view = callView as { kind?: unknown; locations?: unknown } | null
  const location = Array.isArray(view?.locations)
    ? (view.locations[0] as { path?: unknown } | undefined)?.path
    : undefined
  const name = view?.kind === 'read' ? 'read' : tool
  return {
    ...
    time: seq,
    callId: `call-${seq}`,
    call: { name, argsRaw: JSON.stringify({ file_path: location }) },
    callTime: seq - 1,
    content: [],
    isError,
    meta: resultView,
...
function runningCall(callId: string, name: 'read' | 'edit', path: string): RunningToolCall {
  return {
    callId,
    name,
    argsRaw: JSON.stringify({ file_path: path }),
    turn: 1,
    step: 1,
    time: 1,
    subCalls: [],
...
type SnapshotOverrides =
  Partial<Omit<SessionFilesSnapshot, 'chat'>>
  & {
...
function snapshot(overrides: SnapshotOverrides = {}): SessionFilesSnapshot {
  const {
    boundaries = [{ turn: 1, seq: 0 }], nodes = [], runningCalls = [],
    running = false, hasMore = false,
  } = overrides
  return {
    chat: {
      ...EMPTY_CHAT_SNAPSHOT,
      timeline: timeline(boundaries),
...
describe('deriveSessionFiles', () => {
  ...
  it('ignores failed calls and reports a null tool when the call head is gone', () => {
    const orphan = {
      kind: 'tool-result', seq: 10, time: 10, callId: 'orphan', isError: false,
      content: [], callTime: null, subCalls: [],
      meta: diffView(['a.ts'], [{ path: 'a.ts', oldText: null, newText: 'x' }]), call: null,
    } as unknown as ConversationNode
    const model = deriveSessionFiles(snapshot({
      nodes: [
  ...
  it('marks a path a running call is writing and sorts it last', () => {
    const model = deriveSessionFiles(snapshot({
      ...
      runningCalls: [
        runningCall('c1', 'edit', 'live.ts'),
        runningCall('c2', 'read', 'looked.ts'),
      ],
    }))
...
describe('defaultSelection', () => {
  it('prefers the file being written, then the most recently changed', () => {
    const running = deriveSessionFiles(snapshot({
      running: true,
      nodes: [result({ seq: 10, callView: diffView(['settled.ts']) })],
      runningCalls: [runningCall('c1', 'edit', 'live.ts')],
    }))
