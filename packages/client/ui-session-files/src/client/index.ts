...
export function apply(ctx: ClientContext): void {
  ...
  const treeApi: SubagentApi = {
    list: (parentSessionId, signal) => ctx.remote.subagents.list(parentSessionId, signal),
    follow: (request, signal) => ctx.remote.session.follow(request, signal),
    page: (request, signal) => ctx.remote.session.page(request, signal),
  }
  const tree = new SessionTreeController(treeApi)
  ...
  const diffs: ChatFileDiffs = {
    ...
    forTurn(sessionId, turn) {
      const snapshot = snapshotFor(sessionId)
      if (snapshot === undefined) return []
      return sessionFilesOf(snapshot).byTurn.get(turn) ?? []
    },
  }
