/** Descendant file changes derived from raw Session history. */
...
export interface TreeHistoryEntry {
  ...
  /** Optional precomputed presentation carried by older test and transport fixtures. */
  ...
  for (const entry of entries) {
    ...
    if (event.type === 'tool/call') {
      ...
      if (typeof name === 'string') {
        ...
        const args = (event.data as ToolCallData).arguments
        if (typeof args === 'string') calls.set(callId, { name, argsRaw: args })
      }
      continue
    }
    ...
    if (typeof callId !== 'string') continue
    if (!callViews.has(callId)) continue
    const hunks = diffHunks(data.meta ?? entry.view)
    const changed = [...new Set(changedPaths(calls.get(callId) ?? callViews.get(callId) ?? null))]
