/** File-rail data derived from raw tool calls and persisted result metadata. */
...
/** Chat data and Session lifecycle facts used by the file rail. */
...
/** Raw tool-call identity and JSON arguments. */
interface FileCall {
...
/** One validated applied diff hunk. */
interface Hunk {
  readonly path: string
  readonly oldText: string | null
  readonly newText: string
...
function callLocation(call: unknown): { path: string; kind: 'read' | 'edit' } | undefined {
  if (typeof call !== 'object' || call === null) return undefined
  const candidate = call as Partial<FileCall>
  if (typeof candidate.name !== 'string' || typeof candidate.argsRaw !== 'string') return undefined
  let value: unknown
  try {
    value = JSON.parse(candidate.argsRaw)
  } catch {
    // Partial or malformed model JSON has no usable file location.
    return undefined
  }
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return undefined
  const args = value as Record<string, unknown>
  if (candidate.name === 'str_replace_editor') {
    if (typeof args.path !== 'string' || args.path.trim() === '') return undefined
    if (args.command === 'view') return { path: args.path, kind: 'read' }
    if (args.command === 'create' || args.command === 'str_replace') return { path: args.path, kind: 'edit' }
    return undefined
  }
  if (typeof args.file_path !== 'string' || args.file_path.trim() === '') return undefined
  if (candidate.name === 'read') return { path: args.file_path, kind: 'read' }
  if (candidate.name === 'write' || candidate.name === 'edit') return { path: args.file_path, kind: 'edit' }
  return undefined
}

/**
 * Read a mutation call's intended file path.
 * @param call - raw call, or null when pagination omitted its head.
 * @returns intended paths for the shipped mutation tools.
 */
export function changedPaths(call: unknown): readonly string[] {
  if (typeof call === 'object' && call !== null && 'card' in call) {
    const view = call as { card?: unknown; kind?: unknown; locations?: unknown }
    if (view.card === 'diff' || (view.card === 'generic' && view.kind === 'edit')) {
      return locationPaths(view.locations)
    }
    return []
  }
  const location = callLocation(call)
  return location?.kind === 'edit' ? [location.path] : []
}

/**
 * Read the path of a file-read call.
 * @param call - raw call, or null when pagination omitted its head.
 * @returns read paths for the shipped file tools.
 */
export function readPaths(call: unknown): readonly string[] {
  if (typeof call === 'object' && call !== null && 'card' in call) {
    const view = call as { card?: unknown; kind?: unknown; locations?: unknown }
    return view.card === 'generic' && view.kind === 'read' ? locationPaths(view.locations) : []
  }
  const location = callLocation(call)
  return location?.kind === 'read' ? [location.path] : []
}

/** Read valid string paths from an opaque presentation location list. */
function locationPaths(value: unknown): readonly string[] {
...
/**
 * Validate the applied hunks stored in opaque tool-result metadata.
 * @param meta - persisted tool-result metadata.
 * @returns well-formed hunks; malformed entries contribute no file contents.
 */
export function diffHunks(meta: unknown): readonly Hunk[] {
 ...
 */
export function deriveSessionFiles(snapshot: SessionFilesSnapshot): SessionFilesModel {
  ...
  for (const node of snapshot.chat.legacy.nodes) {
    ...
    for (const path of readPaths(node.call)) reads.push(path)
    const hunks = diffHunks(node.meta)
    const changed = node.call === null
      ? [...new Set(hunks.map(hunk => hunk.path))]
      : [...new Set(changedPaths(node.call))]
  ...
  for (const call of snapshot.chat.legacy.runningCalls) {
    for (const path of readPaths(call)) reads.push(path)
 ...
 */
const derived = new WeakMap<ChatSnapshot, SessionFilesModel>()
 ...
 */
export function sessionFilesOf(snapshot: SessionFilesSnapshot): SessionFilesModel {
  const cached = derived.get(snapshot.chat)
  if (cached !== undefined && cached.running === snapshot.running && cached.hasMore === snapshot.hasMore) return cached
  const model = deriveSessionFiles(snapshot)
  derived.set(snapshot.chat, model)
  return model
}
