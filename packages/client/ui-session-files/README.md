...
- **A descendant-only file has no diff in the transcript.** The rail lists it and the merged model holds its segments, but the inline surface hangs off a turn's produced-file chips, and a file only a subagent touched never appears in one. Expanding it under the parent's delegation row is the remaining half of the second stage in the [file panel Agent Note](../../../.agents/notes/implemented/feature/2026-08-26-web-session-file-panel.md); until then, selecting such a row in the rail is inert.
