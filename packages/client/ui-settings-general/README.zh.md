...
手机端从顶部栏进入设置。宽度小于 768px 时，对话框将分区导航放在全宽内容上方的横向滚动行中；关闭控件保留在右上角。

### 「通用」与「布局」分区

「通用」分区承载注册进 `settings.general.item` 的行，「布局」分区承载注册进 `settings.layout.item` 的行；两个分区都没有内置行。功能插件拥有行文案与行为，外壳提供两个平级页面及其 slot。ui-theme 将外观与桌面对话字号贡献到「通用」，将桌面和手机密度与纯净 UI 贡献到「布局」；ui-session-files 将文件可见性与行内 diff 展开方式贡献到「布局」。
