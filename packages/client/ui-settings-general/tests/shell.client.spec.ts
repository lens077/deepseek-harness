...
describe('ui-settings apply', () => {
  ...
  it('projects the section ledger into ordered nav rows with option defaults', async () => {
    ...
    // This package registers the General and Layout sections; feature plugins
    // contribute their rows and any additional sections.
    const GENERAL = { id: 'general', order: 0, label: 'general.nav' }
    const LAYOUT = { id: 'layout', order: 5, label: 'layout.nav' }
    expect(sections.getSnapshot()).toEqual([GENERAL, LAYOUT])
