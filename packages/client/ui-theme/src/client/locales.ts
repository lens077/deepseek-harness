/** `settings.theme` namespace dictionaries (the Appearance and font-size rows' copy). */

/** Simplified Chinese dictionary (the key-set source of truth). */
export const zh = {
  'appearance.title': '外观',
  'appearance.light': '浅色',
  'appearance.dark': '深色',
  'appearance.system': '跟随系统',
  'themeStyles.title': '主题风格',
  'themeStyles.description': '选择一个更有个性的界面配色。',
  'themeStyles.glass': '玻璃态',
  'themeStyles.rainbow': '彩虹糖',
  'themeStyles.deepseek-muse': 'DeepSeek 娘',
  'themeStyles.forest': '森林夜色',
  'themeStyles.sunset': '落日余晖',
  'fontSize.title': '字号大小',
  'fontSize.description': '仅影响会话内容的字号',
  'fontSize.unit': 'px',
  'fontSize.increase': '增大字号',
  'fontSize.decrease': '减小字号',
  'mobile.title': '界面密度',
  'mobile.description': '桌面端和手机端分别保存布局密度，互不覆盖。',
  'desktop.layout': '桌面布局',
  'desktop.layout.large': '宽松',
  'desktop.layout.large.description': '增加侧栏、页头和输入区留白。',
  'desktop.layout.medium': '中等',
  'desktop.layout.medium.description': '平衡信息密度与可读性。',
  'desktop.layout.small': '紧凑',
  'desktop.layout.small.description': '减少留白，在同一屏显示更多内容。',
  'mobile.layout': '手机布局',
  'mobile.layout.large': '大布局',
  'mobile.layout.large.description': '按钮和标题更清晰，任务列表保持紧凑。',
  'mobile.layout.medium': '中等',
  'mobile.layout.medium.description': '紧凑均衡，适合日常使用。',
  'mobile.layout.small': '小布局',
  'mobile.layout.small.description': '显示更多内容，底部导航仅显示图标。',
  'mobile.fontSize': '手机字号',
  'mobile.fontSize.description': '调整手机文字大小；切换布局不会重置字号。',
  'mobile.fontSize.increase': '增大手机字号',
  'mobile.fontSize.decrease': '减小手机字号',
  'mobile.fontSize.value': '{size} px',
  'pureUi.title': '纯净 UI',
  'pureUi.description': '隐藏会话顶部栏，提问以纯文字置顶，输入框收进右下角的圆形按钮，尽可能多地显示回答内容。',
} satisfies Record<string, string>

/** The settings.theme namespace key union. */
export type ThemeKey = keyof typeof zh

/** English dictionary, checked complete against the zh key set. */
export const en = {
  'appearance.title': 'Appearance',
  'appearance.light': 'Light',
  'appearance.dark': 'Dark',
  'appearance.system': 'System',
  'themeStyles.title': 'Theme style',
  'themeStyles.description': 'Choose a more expressive interface palette.',
  'themeStyles.glass': 'Glass',
  'themeStyles.rainbow': 'Rainbow candy',
  'themeStyles.deepseek-muse': 'DeepSeek muse',
  'themeStyles.forest': 'Forest night',
  'themeStyles.sunset': 'Sunset glow',
  'fontSize.title': 'Font size',
  'fontSize.description': 'Only affects conversation content',
  'fontSize.unit': 'px',
  'fontSize.increase': 'Increase font size',
  'fontSize.decrease': 'Decrease font size',
  'mobile.title': 'Interface density',
  'mobile.description': 'Desktop and phone layout densities are stored independently.',
  'desktop.layout': 'Desktop layout',
  'desktop.layout.large': 'Spacious',
  'desktop.layout.large.description': 'Adds space around the sidebar, header, and composer.',
  'desktop.layout.medium': 'Medium',
  'desktop.layout.medium.description': 'Balances information density and readability.',
  'desktop.layout.small': 'Compact',
  'desktop.layout.small.description': 'Reduces spacing to show more content at once.',
  'mobile.layout': 'Phone layout',
  'mobile.layout.large': 'Large',
  'mobile.layout.large.description': 'Clearer controls and titles, with compact task lists.',
  'mobile.layout.medium': 'Medium',
  'mobile.layout.medium.description': 'Compact and balanced for everyday use.',
  'mobile.layout.small': 'Small',
  'mobile.layout.small.description': 'More content, with icon-only bottom navigation.',
  'mobile.fontSize': 'Mobile font size',
  'mobile.fontSize.description': 'Adjust phone text size. Switching layouts keeps this size.',
  'mobile.fontSize.increase': 'Increase mobile font size',
  'mobile.fontSize.decrease': 'Decrease mobile font size',
  'mobile.fontSize.value': '{size} px',
  'pureUi.title': 'Pure UI',
  'pureUi.description': 'Hide the Session header, pin the question as plain text, and fold the composer behind a round button so answers take the whole column.',
} satisfies Record<ThemeKey, string>
