...
Open the Plugins section in Settings and select the **Plugin configuration** tab to edit the host-plane plugins this deployment composes. The cards appear in this order: the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), subagent model selection (`subagent-model-selection`), and the DeepSeek search provider (`web-search-deepseek`). This package also owns the model-routing switch, which it places on the Models page rather than here.
...
The model-routing switch registers into the Models page's `settings.models.footer` seat and renders only while a `dsh-model-router` provider serves the `model-routing` namespace. It has no drafts: one toggle is one write of `enabled`, an absent value reads as on, and a write the Host did not accept reports itself while the switch keeps the stored position. Turning it off stops every prompt from consulting the router and returns a Session left on a routed model to the person's route at its next prompt; the switch never unmounts the provider.
