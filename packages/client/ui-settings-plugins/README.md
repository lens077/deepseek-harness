...
Open the Plugins section in Settings and select the **Plugin configuration** tab to edit the host-plane plugins this deployment composes. The cards appear in this order: the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), model routing (`model-routing`), subagent model selection (`subagent-model-selection`), and the DeepSeek search provider (`web-search-deepseek`).
...
The Model routing card renders only while a `dsh-model-router` provider is mounted. Its one switch stages `enabled` as text and saves through the ordinary path; an absent value reads as on. Turning it off stops every prompt from consulting the router and returns a Session left on a routed model to the person's route at its next prompt; the switch never unmounts the provider.
