...
打开设置中的「插件」分区并选择**插件配置**标签页，即可编辑本部署所组装的宿主平面插件。卡片依次为 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）、subagent 模型选择（`subagent-model-selection`）以及 DeepSeek 搜索提供方（`web-search-deepseek`）。本包还拥有模型路由开关，但把它放在「模型」页而不是这里。
...
模型路由开关注册到「模型」页的 `settings.models.footer` 席位，只在 `dsh-model-router` 提供方提供 `model-routing` namespace 时渲染。它没有草稿：拨一次就是一次 `enabled` 写入，值缺省视为开启，Host 未接受的写入会自行报告而开关保持已保存的位置。关闭后每条提示词都不再咨询路由器，停留在被路由模型上的会话会在下一条提示词时回到用户的路由；该开关绝不会卸载提供方。
