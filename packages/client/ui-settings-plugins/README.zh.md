...
打开设置中的「插件」分区并选择**插件配置**标签页，即可编辑本部署所组装的宿主平面插件。卡片依次为 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）、模型路由（`model-routing`）、subagent 模型选择（`subagent-model-selection`）以及 DeepSeek 搜索提供方（`web-search-deepseek`）。
...
模型路由卡片只在挂载了 `dsh-model-router` 提供方时渲染。它唯一的开关把 `enabled` 暂存为文本并通过常规路径保存；值缺省视为开启。关闭后每条提示词都不再咨询路由器，停留在被路由模型上的会话会在下一条提示词时回到用户的路由；该开关绝不会卸载提供方。
