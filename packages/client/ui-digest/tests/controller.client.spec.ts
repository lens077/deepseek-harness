...
describe('InboxController', () => {
  ...
  it('files a todo once per question and marks the session handled', async () => {
    const { remote, calls } = remoteStub()
    const controller = new InboxController(remote)
    controller.receive(inbox({ todos: [
      todo('open', 'a', { questionSeq: 2 }),
      todo('done', 'a', { questionSeq: 3, status: 'done' }),
      todo('whole', 'b', { questionSeq: null }),
    ] }))
    // An open todo on the same question is reused; only the handled mark is written.
    await expect(controller.fileTodo({ sessionId: A, questionSeq: 2, text: 'x' })).resolves.toEqual({ ok: true })
    expect(calls).toEqual(['setHandled'])
    expect(remote.setHandled).toHaveBeenLastCalledWith({ sessionId: A, handled: true })
    // A done todo, another question, or another session does not count.
    calls.length = 0
    controller.receive(inbox({ todos: [todo('done', 'a', { questionSeq: 3, status: 'done' }), todo('whole', 'b', { questionSeq: null })] }))
    await expect(controller.fileTodo({ sessionId: A, questionSeq: 3, text: 'x' })).resolves.toEqual({ ok: true })
    await expect(controller.fileTodo({ sessionId: A, questionSeq: null, text: 'x' })).resolves.toEqual({ ok: true })
    expect(calls).toEqual(['addTodo', 'setHandled', 'addTodo', 'setHandled'])
    // A failed create leaves the handled mark untouched.
    const addTodo = remote.addTodo as ReturnType<typeof vi.fn>
    addTodo.mockResolvedValueOnce(ok({ ok: false, error: { code: 'text-blank' } }))
    await expect(controller.fileTodo({ sessionId: A, questionSeq: 9, text: ' ' })).resolves.toMatchObject({ ok: false, error: { code: 'text-blank' } })
    expect(addTodo).toHaveBeenCalledTimes(3)
    expect(remote.setHandled).toHaveBeenCalledTimes(3)
  })
