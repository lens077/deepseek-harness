...
describe('ui-digest host', () => {
  ...
  it('registers the pin section with defaults and row bounds, and disposes it with the fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({ enabled: true, sidebarArea: true, sidebarRows: 5, autoPinStatuses: ['running', 'completed'], digestSection: true })
    await ctx.settings.update(ns, { enabled: false, sidebarRows: 8, digestSection: false })
    expect(ctx.settings.get(ns)).toEqual({ enabled: false, sidebarArea: true, sidebarRows: 8, autoPinStatuses: ['running', 'completed'], digestSection: false })
    await ctx.settings.update(ns, { sidebarRows: 'auto', autoPinStatuses: ['failed'] })
    expect(ctx.settings.get(ns)).toMatchObject({ sidebarRows: 'auto', autoPinStatuses: ['failed'] })
    ...
    await expect(ctx.settings.update(ns, { sidebarRows: 'fit' })).rejects.toThrow()
    await expect(ctx.settings.update(ns, { autoPinStatuses: ['paused'] })).rejects.toThrow()
