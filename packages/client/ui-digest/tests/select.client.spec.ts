...
describe('selectInbox classification', () => {
  it('sections unread, seen, running, waiting, and failed rows in that order', () => {
    const rows = [
      row('seen'),
    ...
    const selection = selectInbox(rows, [], marks, options({
      pendingSessionIds: new Set(['waiting' as SessionId]),
    }))
    ...
    expect(selection.failedCount).toBe(2)
    expect(selection.unreadCount).toBe(1)
    expect(selection.seenCount).toBe(1)
    expect(selection.runningCount).toBe(2)
    expect(selection.sections[2]?.items[1]).toMatchObject({
    ...
    const failed = selection.sections[4]?.items[0]
...
describe('selectInbox window and filters', () => {
  it('admits running and actionable rows regardless of the window but bounds context rows', () => {
    const old = NOW - 3 * DAY
    const rows = [
    ...
    expect(sectionKeys(since)).toEqual(['unread:oldUnread', 'seen:newSeen', 'running:oldRunning'])
