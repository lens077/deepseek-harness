...
export interface DigestPanelInjected {
  ...
  /** Create one todo; the todo tab's free-text form. */
  addTodo: (request: InboxAddTodoRequest) => Promise<InboxActionResult>
  /** Move an inbox row to the todo list: reuse an open todo on the same question, then mark the Session handled. */
  fileTodo: (request: InboxAddTodoRequest) => Promise<InboxActionResult>
