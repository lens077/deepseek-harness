...
export interface InboxSelection {
  ...
  waitingCount: number
  /** Rows whose last turn ended any way but `completed`, snoozed and handled excluded. */
  failedCount: number
  /** Rows in the unread section: finished after the user's last seen mark, snoozed and handled excluded. */
  unreadCount: number
  /** Rows in the seen section: finished and seen but not handled, snoozed excluded. */
  seenCount: number
  runningCount: number
  snoozedCount: number
...
function classify(
  ...
  if (waiting) return { category: 'needsYou', unread: false }
...
function buildItems(
  ...
  for (const summary of sessions) {
    ...
    const waiting = pendingSessionIds?.has(summary.id) === true
    const classified = classify(summary, digest, mark, now, waiting)
 ...
 *
 * Running, waiting, failed, unread, and pinned rows are admitted regardless
 * of the window: in-flight work is current by definition, and the rest are
...
export function selectInbox(
  ...
  for (const item of items) {
    ...
    if (actionable) {
      ...
      if (item.category === 'failed') failedCount += 1
      if (item.category === 'unread') unreadCount += 1
    }
