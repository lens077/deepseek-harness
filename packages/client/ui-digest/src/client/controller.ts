...
export class InboxController extends SnapshotController<InboxView> {
  ...
  /**
   * Move one inbox row into the todo list: create the todo unless an open todo
   * already addresses the same Session and question, then mark the Session
   * handled so its row leaves the inbox. A second press on the same card is
   * therefore idempotent instead of growing the list.
   * @param request - target Session, optional question seq, and text for the todo when none exists.
   * @returns the settled result; a failed create leaves the handled mark untouched.
   */
  async fileTodo(request: InboxAddTodoRequest): Promise<InboxActionResult> {
    const exists = this.getSnapshot().snapshot.todos.some(todo => (
      todo.status === 'open' && todo.sessionId === request.sessionId && todo.questionSeq === request.questionSeq
    ))
    if (!exists) {
      const added = await this.addTodo(request)
      if (!added.ok) return added
    }
    return this.setHandled(request.sessionId, true)
  }
