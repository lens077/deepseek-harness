...
// Type-only: pulls ui-conversation's Context merge (ctx.conversation).
import type {} from '@deepseek-ai/dsh-client-ui-conversation/client'
// Type-only: pulls ui-chat's optional cross-session question reveal service.
...
export const inject = ['slots', 'sessions', 'workspaces', 'uiWorkspace', 'uiSession', 'locale', 'remote', 'remote.session', 'remote.sessionInbox', 'remote.projectTodos']
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['settingsScope'], (settingsCtx) => {
    ...
    settingsCtx.slots.inject('settings.section', () => settingsCtx.slots.register({
      ...
      id: 'digest',
      // Beside the shipped sections, just ahead of the project todos page.
      order: 44,
      locale: NS,
  ...
  ctx.inject(['documentBadge'], (badgeCtx) => {
    ...
    badgeCtx.effect(() => ctx.uiSession.pendingInteractions.subscribe(recount), 'ui-digest: badge from pending interactions')
  ...
  // The session browser's menus file todos through this seat; the wording
  ...
  ctx.slots.inject('sidebar.nav.entry', () => ctx.slots.register({
    ...
    locale: NS,
    store,
    inject: (): DigestNavEntryInjected => ({ hooks: { inbox: controller, navSettings: navSettings.view } }),
  }, DigestNavEntry))
  ...
  ctx.slots.inject('center.overlay', () => ctx.slots.register({
    ...
    inject: (actions): DigestPanelInjected => {
      ...
      return {
        ...
        fileTodo: request => controller.fileTodo(request),
