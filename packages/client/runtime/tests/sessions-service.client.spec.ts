...
describe('fork', () => {
  ...
  it.each(['sibling', 'nested'] as const)('forwards %s placement through the manager to the Host', async (placement) => {
    const b = bench()
    await feedList(b, [{ id: 'source', cwd: '/work' }])
    b.api.onFork = () => Promise.resolve(ok({ sessionId: sid('child') }))

    await expect(b.svc.fork({ sessionId: sid('source'), placement })).resolves.toBe('child')

    expect(b.api.callsOf('session.fork')).toEqual([{ sessionId: 'source', placement }])
  })
