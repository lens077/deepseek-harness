...
Below 768px, `Menu` uses at least 44px rows, wraps labels, and expands submenus inside its scrolling card. Portaled menus follow list-size changes as well as scroll and viewport resize. `closeOnPointerLeave` applies only to non-touch pointers, so lifting a finger does not dismiss the menu. Standard `Modal` dialogs fit the dynamic viewport, scroll their content, and keep footer actions reachable; close and footer controls have at least 44px touch targets.
