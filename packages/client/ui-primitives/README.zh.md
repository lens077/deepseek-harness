...
宽度小于 768px 时，`Menu` 的行高至少为 44px，标签可换行，子菜单在可滚动的菜单卡片内展开。采用 portal 的菜单会随列表尺寸、滚动位置和视口尺寸变化重新定位。`closeOnPointerLeave` 仅作用于非触控指针，因此抬起手指不会关闭菜单。标准 `Modal` 对话框限制在动态视口内，内容可滚动，底部操作始终可访问；关闭与底部按钮的触控区域至少为 44px。
