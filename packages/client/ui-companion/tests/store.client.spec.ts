...
describe('companion viewing state', () => {
  it('cycles from quiet company to greeting, rest, and awake', () => {
    ...
    expect(store.getSnapshot()).toEqual({ mood: 'idle', collapsed: false, paused: false, position: null })
  ...
  it('collapses without losing rest or motion preferences', () => {
    ...
    expect(store.getSnapshot()).toEqual({ mood: 'sleeping', collapsed: true, paused: true, position: null })
    store.actions.toggleCollapsed()
    store.actions.toggleMotion()
    expect(store.getSnapshot()).toEqual({ mood: 'sleeping', collapsed: false, paused: false, position: null })
  })
