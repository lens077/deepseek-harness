...
逐级进入文件夹、直接编辑路径，或用前缀过滤最后一栏；宿主标记的隐藏条目默认不显示，直到页脚开关揭开。宽度小于 768px 时只显示当前目录层级：列表纵向滚动，长名称换行，通过面包屑返回上级，无需横向滚动分栏。页脚操作始终位于视口内。**新建文件夹**打开一个嵌套创建对话框，目标为选中的文件夹，并选中它创建出来的那个；**打开**采纳选中的文件夹，没有选中时回落到当前层级。确认一个目录即为选中的路径；关闭对话框即为取消。
