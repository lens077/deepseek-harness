...
The standalone browser fixture has no dynamic Host plugin execution. It answers `dynamicCordisRunner/inventory` with an empty successful inventory and `dynamicCordisRunner/syncInspectManifest` with a successful `null` value; unknown RPC endpoints still reject rather than returning generic success.
