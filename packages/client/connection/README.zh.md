...
独立浏览器 fixture（测试前置数据）不执行动态 Host 插件。它为 `dynamicCordisRunner/inventory` 返回成功的空清单，为 `dynamicCordisRunner/syncInspectManifest` 返回成功的 `null` 值；未知 RPC 端点仍然拒绝，不返回通用成功结果。
