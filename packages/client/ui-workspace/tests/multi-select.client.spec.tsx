...
function mountRows(names: readonly string[], overrides: Partial<WorkspaceBrowserProps> = {}) {
  ...
  const props: WorkspaceBrowserProps = {
    ...
    replaceSessionDirectories: vi.fn<WorkspaceBrowserProps['replaceSessionDirectories']>(async (_sessionId, additionalDirectories) => ({
    ...
    deleteSession: vi.fn<WorkspaceBrowserProps['deleteSession']>(async sessionId => [sessionId]),
    addTodos: vi.fn(),
    todosAvailable: () => false,
    setPinned: vi.fn(async () => undefined),
    setPinnedSidebarRows: vi.fn(async () => undefined),
    setPinnedAutoStatuses: vi.fn(async () => undefined),
    setSessionMembership: vi.fn<WorkspaceBrowserProps['setSessionMembership']>(async (workspaceId, sessionIds) => workspace(workspaceId, [...sessionIds])),
    insertWorkspaceBefore: vi.fn(async () => {}),
    insertSessionBefore: vi.fn(async () => {}),
    createWorkspace: vi.fn(async () => workspace('created', [])),
    useDirectoryFlow: bindSnapshotSelector({ getSnapshot: () => true, subscribe: () => () => {} }),
    useSessionDirectoryFlow: bindSnapshotSelector({ getSnapshot: () => true, subscribe: () => () => {} }),
    useHostInfo: selector => selector({ home: '/home/test', isLoopback: true }),
