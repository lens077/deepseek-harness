...
Session 行在源会话最后一个已完成轮次处提供两种 fork 位置，在客户端递增继承的持久化标题后再打开子会话；尾部半角或全角括号编号会原样式递增，无编号标题追加 ` (1)`。**复制为平级会话**在同一 Workspace 中添加顶层同级行。**新建嵌套子会话**把子会话放入源会话在该 Workspace 下的可展开分支；嵌套只改变 Workspace 显示记账，两种模式都会持久保留 Session 谱系。Fork 或改名失败都不会改变当前选中项，改名失败时已创建的子会话仍会留在列表中。
...
无。浏览器界面本身不贡献提示词文本：目录替换提交后会追加会话事件，沙箱策略归属方在下一份运行时上下文快照中渲染该事件。

#### KV Cache 影响

替换成功后，会改变保留历史之后的下一份归属方渲染快照；该 UI 包既不组装也不发送提供方请求。

## 已知限制与暂缓事项

- **没有模糊内容搜索或事件深链接**：内容后端采用字面 token／短语匹配，选择结果会打开 Session，而不是匹配的事件。
- **没有 Session 删除功能**：取消归档只会恢复可见性，不会删除数据；删除 Workspace 注册记录同样不会删除 Session。
