...
- **Pinned shortcuts are positions, not Sessions** — a chord opens whatever row sits at its position when pressed; the positions follow recency, so a newer Session entering the area moves every row below it down one position, and opening a status-listed row dismisses it and moves the rows below up. The conflict warning covers the common browser and desktop bindings of the detected platform, not every extension, terminal, or window-manager binding a machine may carry.
