...
与 `ui-conversation` 一起挂载本插件；已完成轮次随即以产出文件行收尾，位于收尾消息正文与其动作页脚之间。`ui-session-files` 提供已记录 hunk 时，标签项会切换修改前后的行内对照，对照头部可打开该文件；没有已记录 hunk 的标签项会直接打开文件。两者都经属主的 `openFile` 打开——chat 视图把它路由到右侧 Sidebar 作为一个文本预览 tab——相对路径按会话 cwd 解析。该行不提供文件夹动作：Sidebar 没有目录形态，因此省略文件的余数只是一个标签。

### 该行

该行通过 CSS 容器宽度档位响应式展示至多六个文件标签项。Flexbox 负责收缩文件名并用 ellipsis 省略，CSS 为未展示路径选择匹配的本地化 `+ N 个文件` 标签；完整路径仍保留在 `title` 中，标签项一行不执行 JavaScript 布局观察，也不提供横向滚动。桌面端的共享 `all` / `single` / `none` 偏好分别默认展开所有可对照文件、仅含一个文件的 Turn，或全部不展开。手机端的差异对照保持收起，直到点击对应文件标签项；文件或偏好更新不会自动展开已收起的对照。手机端和桌面端分别保留手动展开与收起的选择。create 没有修改前内容，因此默认保持收起。
