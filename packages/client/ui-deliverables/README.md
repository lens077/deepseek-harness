...
Mount this plugin alongside `ui-conversation`; a finished turn then ends with the produced-files row between the closing message's body and its action footer. When `ui-session-files` supplies recorded hunks, a chip toggles an inline before/after comparison whose header opens the file; a chip without recorded hunks opens the file directly. Both open through the owner's `openFile`, which the chat view routes to the right Sidebar as a text-preview tab, with relative paths resolved against the session cwd. The row offers no folder action: the Sidebar has no directory form, so an omitted-file remainder is a label only.
