...
When a [`dsh-model-router`](../../llm/model-router/README.md) provider is mounted, prompt admission consults it after attachment admission and before queueing each human prompt. The controller derives the baseline route the person owns from the `modelSelection` projection, hands the router every provider/model route the live registry advertises, applies the answer to the next prompt assembly through the Session-local selection, and appends one log-only `model/route` event carrying the baseline, the applied route, the reason, and the deciding rule. A route change is applied only when at least two routes are configured, the answer names one of them, an image prompt lands on a route declaring image input, the Session's `ctx.tokenMeter` measurement fits the route's context window when both are known, and its effort resolves; otherwise the answer degrades to its effort on the baseline route. An effort the baseline model rejects through `ctx.llm.resolveCallConfig()`, or a router failure, applies the baseline and records why. Routing never rejects the prompt, never appends `model/selection`, and never saves the deployment default; `session.selectModel` remains the only path that does both. While the router's `model-routing.enabled` setting is off, the router is not consulted and a Session still on a routed selection is returned to its baseline. When a request on a routed route ends in a failure no downstream `agent/request-error` listener retries, the controller moves the step's selection back to the baseline, records the fallback as `model/route`, and answers with a retry; a failure on the baseline route itself is left to end the turn.
...
- Model routing switches off deployment-wide through `model-routing.enabled` and has no per-Session opt-out; the wire `modelSelection` view shows a routed route only once its request header lands.
