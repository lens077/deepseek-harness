...
- Model routing has no per-Session opt-out: with a router mounted, every human Web prompt is routed, and the wire `modelSelection` view shows a routed effort only once its request header lands.
