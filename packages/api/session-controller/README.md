...
When a [`dsh-model-router`](../../llm/model-router/README.md) provider is mounted, prompt admission consults it before queueing each human prompt. The controller derives the baseline route the person owns from the `modelSelection` projection, applies the router's answer to the next prompt assembly through the Session-local selection, and appends one log-only `model/route` event carrying the baseline, the applied route, the reason, and the deciding rule. Only the reasoning effort may change: an answer naming another provider or model, an effort the exact model rejects through `ctx.llm.resolveCallConfig()`, or a router failure applies the baseline and records why. Routing never rejects the prompt, never appends `model/selection`, and never saves the deployment default; `session.selectModel` remains the only path that does both.
