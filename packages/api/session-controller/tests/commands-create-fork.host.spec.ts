...
/** Controller double: seeded creation goes through the registry so `create` spies observe the fork. */
function controllerAgents(ctx: Context, overrides: object = {}): ApiSessionAgentController {
  return {
    ensureSession: () => Promise.resolve(),
    createSeeded: (options: CreateAgentOptions) => ctx.agents.create(options).then(handle => handle.agent),
