...
当组合中挂载了 [`dsh-model-router`](../../llm/model-router/README.zh.md) 提供方时，prompt 准入会在每条人类提示词入队前询问它。控制器从 `modelSelection` 投影推导用户拥有的基线路由，通过 Session 本地选择把路由器的答案应用到下一次提示词组装，并追加一条仅记录日志的 `model/route` 事件，携带基线、实际应用的路由、原因与决定性的规则。只允许改变推理强度：答案若指定了其他 provider 或 model、指定了确切模型经 `ctx.llm.resolveCallConfig()` 拒绝的强度，或路由器本身失败，都会应用基线并记录原因。路由绝不拒绝提示词、绝不追加 `model/selection`、也绝不保存部署默认值；`session.selectModel` 仍是唯一同时做这两件事的路径。
