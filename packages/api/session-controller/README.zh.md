...
当组合中挂载了 [`dsh-model-router`](../../llm/model-router/README.zh.md) 提供方时，prompt 准入会在附件准入之后、每条人类提示词入队之前询问它。控制器从 `modelSelection` 投影推导用户拥有的基线路由，把实时注册表公布的每条 provider/model 路由交给路由器，通过 Session 本地选择把答案应用到下一次提示词组装，并追加一条仅记录日志的 `model/route` 事件，携带基线、实际应用的路由、原因与决定性的规则。只有当至少配置了两条路由、答案指名其中之一、带图片的提示词落在声明图片输入的路由上、Session 的 `ctx.tokenMeter` 测量在两者都已知时不超过该路由的上下文窗口、且其强度可解析时，才应用路由变化；否则答案降级为在基线路由上应用其强度。基线模型经 `ctx.llm.resolveCallConfig()` 拒绝的强度，或路由器本身失败，都会应用基线并记录原因。路由绝不拒绝提示词、绝不追加 `model/selection`、也绝不保存部署默认值；`session.selectModel` 仍是唯一同时做这两件事的路径。路由器的 `model-routing.enabled` 设置关闭期间不咨询路由器，仍停留在路由选择上的 Session 会被送回基线。当被路由路由上的请求以没有任何下游 `agent/request-error` 监听器重试的失败结束时，控制器把该步骤的选择挪回基线、把回退记录为 `model/route` 并回答重试；基线路由自身的失败则照旧结束该轮。
...
- 模型路由的开关是部署级的 `model-routing.enabled`，没有按 Session 的退出开关；线上的 `modelSelection` 视图只有在其请求 header 落地后才显示被路由到的路由。
