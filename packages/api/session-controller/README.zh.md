...
- 模型路由没有按 Session 的退出开关：挂载路由器后，每条人类 Web 提示词都会被路由；线上的 `modelSelection` 视图只有在其请求 header 落地后才显示被路由到的强度。
