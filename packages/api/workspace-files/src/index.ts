...
export class WorkspaceFiles extends TypertRemoteService {
   ...
   * the same whichever instance answers. The primary root is the Session cwd;
   * additional writable directories do not widen the file API's containment.
   */
  private workspaceRootOf(agent: Agent): string {
    return this.ctx.sandboxPolicy.resolve({ session: agent.session }).workspaceRoots[0]
  }
