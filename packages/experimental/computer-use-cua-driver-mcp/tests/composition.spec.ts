...
async function load(mode?: string): Promise<{ ctx: Context; root: string; model: ScreenshotModel; failure?: unknown }> {
  ...
  // A rejected entry rolls its whole include group out of the Loader tree, so the
  // failure is returned beside the surviving Context instead of aborting the test.
  const failure = await ctx.loader.create({ name: 'cordis:include', config: { path: pathToFileURL(configPath).href } })
    .then(async () => ctx.loader.await())
    .then(() => undefined, (error: unknown) => error)
  return { ctx, root, model, ...failure === undefined ? {} : { failure } }
}
...
describe('installed Cua Driver Loader composition', () => {
  ...
  it('rejects the composition and releases ownership when installed driver initialization fails', async () => {
    const { ctx, root, failure } = await load('fail')
    expect(String(failure)).toContain('initial connection or tool synchronization failed')
    expect([...ctx.loader.entries()].find(item => item.options.id === 'computer-use-driver')).toBeUndefined()
