...
describe('approveEscalation', () => {
  ...
  it('a request the effective mode already covers is granted as a no-op and never asks', async () => {
    const seen: unknown[] = []
  ...
  it('an unrecognized mode pairing fails closed with the strictly-wider text and never asks', async () => {
    const seen: unknown[] = []
    const spy = ingredients({ approver: approver('allowed-once', r => seen.push(r)) })
