...
`command`, `workdir`, and `timeoutMs` are resolved against the executor's config defaults via `ctx.shell.resolve()` before execution. The workdir default is applied in the tool layer before `resolve()`: a standing sandbox policy uses canonical `workspaceRoots[0]`, otherwise the calling agent's `session.header.cwd` applies. The per-session base must come from `exec.agent`, since N sessions share one executor; only when neither is available does the executor fall back to its own config / `process.cwd()`. Additional roots extend write access but never change relative-workdir resolution.
...
- **Additional roots do not retarget relative workdirs** — `workspaceRoots[0]` remains the canonical workdir base; callers must pass an absolute `workdir` to start under an additional directory.
