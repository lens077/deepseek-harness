...
The exported `createMcpToolDefinition(ctx, options)` builds that same unregistered `ToolDefinition` from one upstream tool plus a caller-owned `call(args, execution)` callback, so a provider that speaks MCP result JSON without this plugin's transport — the [native Cua Driver provider](../../experimental/computer-use-cua-driver-native/README.md), for one — reuses the canonical values, structured output, and durable image projection. Registration, provider lifetime, deadlines, and transport stay with that caller; a callback result that is not an MCP result object fails the tool call.
