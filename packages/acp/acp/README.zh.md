...
| 方法 | 行为 |
|---|---|
| `initialize` | 协商受支持的版本。只有挂载持久附件存储，且配置的确切提供方／模型解析后明确支持图片输入时，才公布图片提示词能力；音频与嵌入上下文保持 false。公布 `sessionCapabilities.additionalDirectories`；不公布编辑器、终端、文件系统或 MCP 能力。 |
| `authenticate` | 空操作，因为服务器不公布身份验证方法。 |
| `session/new` | 以绝对路径作为主要 `cwd` 创建新 agent。绝对且现有的 `additionalDirectories` 会先规范化；非空有效列表会在 Agent 发布前记录为初始完整列表会话快照，没有该事件则表示空列表；非空 `mcpServers` 会被拒绝。 |
...
一个连接可以拥有多个会话。桥接层以带品牌的会话 id 作为记录键，并在路由事件或权限请求前检查 agent 是否为同一对象。每个会话都有独立的提示词槽位、主要 cwd 与附加根目录集合、取消路径和资源释放器。
...
沙箱策略归属方会在运行时上下文快照中渲染规范化的主要 cwd 和每个已接受的附加目录。ACP 元数据本身仍不会进入请求。

#### Token 影响

根目录列表会在首次请求中贡献一条有界上下文消息；只有另一项会话级写入替换该列表时，内容才会变化。

#### KV Cache 影响

初始快照属于首次请求前缀；后续替换会在保留的历史之后追加一份取代先前状态的运行时上下文快照。
...
- **仅光栅图片且不支持 MCP 服务器**：图片提示词要求持久存储以及明确声明支持图片输入的确切路由；只接受 PNG、JPEG、WebP 和 GIF。音频、嵌入资源和非空 MCP 服务器都会被拒绝；资源链接只会展平为文本引用，不会获取其内容。
