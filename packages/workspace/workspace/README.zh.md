...
- `Workspace.attachSession(id)` 与 `attachSessions(ids)` 会在写入前，对照 Workspace 路径验证实时或已持久化 Session 头部的 cwd，再按选区顺序前置新 id。未知 Session、缺失／无法解析／非目录的 cwd 值和不匹配情况都会拒绝完整调用且不写入。`detachSession` 与 `detachSessions` 只移除 Workspace 记账，清理无效嵌套位置，并提升父会话被移除后仍然存在的子会话。
...
- `ctx.workspaceRegistry.archiveSession(id)`/`archiveSessions(ids)`/`unarchiveSession(id)`/`archivedSessionIds`：覆盖在 Workspace 记账之上的注册表级全局归档集合。被归档的 Session 会从活跃分组视图中消失，但其 Session 日志和 `sessionIds` 席位保持不变，因此取消归档后仍回到原位置。归档接受任何实时或已持久化的 Session（无论已记账还是 Ungrouped），对已归档的 id 直接完成且不写入，并拒绝未知 id；批量归档会在一次状态写入前验证全部 id。取消归档会移除已归档的 id；若 id 不在集合中（包括未知 id），则直接完成且不写入，使过期的归档条目仍可恢复。两项变更均与注册表操作串行执行，并保留集合的 Host 顺序。在该字段出现之前写入的状态会解析为空集合。
...
- `ctx.workspaceRegistry.deleteSession(id, retire?)` 会计算完整的持久化后代谱系，在变更前拒绝不安全的实时目标，写入可恢复的子级优先删除标记，删除全部持久化工件，再清理 Workspace 记账、归档成员关系和嵌套位置。可选的 `RetireSessionsForDelete` 回调是 ApiProxy 持有的能力，只能停用由其确切持有的空闲 Agent handle；缺少或不足的所有权会拒绝实时删除。
...
- 破坏性的文件夹移除仍未提供；删除 Workspace 注册记录绝不能替代永久 Session 删除（参见[决策记录](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.zh.md)）。
