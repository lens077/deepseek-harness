...
describe('agent loop', () => {
  ...
  it('spends no model call on a removed waking message even while a runtime-context section is registered', async () => {
    ...
    const agent = await ctx.agentLoop.create(SessionId('removed-wake-with-context'), { provider: 'mock', model: 'mock' })
    ...
    expect(adapter.requests).toEqual([])
    expect(agent.session.snapshotEvents().filter(e => e.type === 'step/start')).toHaveLength(0)
    expect(agent.session.snapshotEvents().filter(e => e.type === 'user/message')).toHaveLength(0)
  })
