...
- **缺失的图片对象降级为文本**——向支持图片的路由分发前，请求组装会对每个不同的图片调用已挂载附件提供方的 `imageAvailable()` 探测；存储对象已缺失的引用（例如 harness home 被删除后）会变成一个确定性的 `missingImageText()` 占位符，指出该附件并请求重新上传，因此对象已丢失的会话日志仍能继续运行，而不是以 `ATTACHMENT_NOT_FOUND` 失败。
