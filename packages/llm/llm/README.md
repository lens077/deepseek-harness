...
- **Absent image objects degrade to text** — before dispatch to an image-capable route, request assembly probes the mounted attachment provider's `imageAvailable()` for every distinct image; a reference whose stored object is missing (for example after the harness home was deleted) becomes one deterministic `missingImageText()` placeholder naming the attachment and asking for re-upload, so a session log that outlived its objects still runs instead of failing with `ATTACHMENT_NOT_FOUND`.
