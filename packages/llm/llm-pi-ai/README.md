...
Generated content affects later inputs only after the loop records it. pi-ai's `output` count already includes reasoning tokens; when pi-ai exposes a reasoning breakdown, the adapter preserves it, including `0`, without adding it to `outputTokens` or `totalTokens`. The adapter omits `reasoningTokens` when pi-ai omits the breakdown.
