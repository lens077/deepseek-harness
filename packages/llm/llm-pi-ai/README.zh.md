...
生成内容只在 loop 记录后才影响后续输入。pi-ai 的 `output` 计数已经包含推理 token；当 pi-ai 暴露推理明细时，适配器会保留该明细（包括 `0`），但不会将其再次加到 `outputTokens` 或 `totalTokens`。pi-ai 省略该明细时，适配器也会省略 `reasoningTokens`。
