...
Permanent deletion holds `BEGIN IMMEDIATE`, validates schema identity, deletes the selected `sessions` row, and commits after the foreign-key cascade removes every physical event row. An absent row returns `false`; any failure rolls the transaction back.
