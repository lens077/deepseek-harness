...
`usageLedger` excludes the exact fork-inherited event prefix. Model rows count usage-reporting requests separately from distinct usage-reporting steps; a retry routed to a different model can therefore have requests but zero steps. The ledger retains observed buckets when their allocation is incomplete, but hides route pricing and the complete total. A missing usage report, contradictory counters, or a closed model step without a settlement also hides the total. An exact provider total can prove omitted cache buckets are zero; omission alone cannot.
