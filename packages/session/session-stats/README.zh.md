...
`usageLedger` 排除精确的 fork 继承事件前缀。模型行分别统计上报用量的请求数和不同逻辑步骤数，因此切换到另一模型的重试可以有请求数而步数为零。分配不完整时账本保留已观察到的分桶，但隐藏完整总额，同时保留已定价路由的部分观察费用。缺失用量、计数矛盾或没有结算事件的已关闭模型步骤也会隐藏完整总额。提供方的精确总量可证明省略的缓存分桶为零；配置的 TokenTracker 兼容模式也会将缺失的可选缓存分桶视为零。
