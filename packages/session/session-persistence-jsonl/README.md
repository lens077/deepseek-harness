...
- **Permanent deletion.** `delete(id)` removes the complete encoded session directory, including future backend-owned entries, without following a symbolic link or junction. It retains the shared project directory and synchronizes that directory on POSIX after removal.
