...
- **Deletion keeps the lock inode on POSIX** — `delete(id)` acquires the Session write lock, removes every generation and Session-owned artifact, fsyncs the surviving directory, and leaves `session.lock` in place so deleting the path cannot split the advisory-lock identity.
