...
| `delete(id): Promise<boolean>` | Permanently remove the complete backend-owned artifact after the live owner retires. `true` means materialized bytes existed; `false` means the id was absent or only an unmaterialized create intent existed. The started operation is not cancellable. |
