...
- **No seam-level redaction rules** — with no `session-telemetry/record` listener mounted, records reach a backend exactly as captured. A custom backend that forwards them unchanged may therefore export credentials embedded in file contents or command output. The shipped OTel backend independently defaults to metadata-only output; other backends own an equally explicit policy.
