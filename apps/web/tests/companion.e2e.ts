...
describe('web companion', () => {
  ...
  it('moves the phone character with a touch gesture without activating its mood', async () => {
    ...
    const handle = await pet.getByRole('button', { name: 'Say hello to your companion' }).boundingBox()
  ...
  it('shows an anchored today/week/month preview and keeps detailed accounting reachable', async () => {
    const trigger = page.getByRole('button', { name: 'Usage overview', exact: true })
    await trigger.click()
    const preview = page.locator('[data-usage-preview]')
    await preview.waitFor()
    expect(await preview.getByRole('button', { name: 'Today', exact: true }).getAttribute('aria-pressed')).toBe('true')
    expect(await page.locator('[aria-modal="true"]').count()).toBe(0)
    const week = preview.getByRole('button', { name: 'This week', exact: true })
    await week.focus()
    await week.press('Enter')
    expect(await week.evaluate(el => el === document.activeElement)).toBe(true)
    await expect.poll(() => preview.locator('[data-usage-period]').getAttribute('data-usage-period')).toBe('week')
    await preview.getByRole('button', { name: 'This month', exact: true }).click()
    await expect.poll(() => preview.locator('[data-usage-period]').getAttribute('data-usage-period')).toBe('month')
    await page.setViewportSize({ width: 320, height: 844 })
    await expect.poll(() => page.locator('[data-mobile]').count()).toBeGreaterThan(0)
    await preview.waitFor({ state: 'visible' })
    const bounds = await preview.boundingBox()
    expect(bounds!.width).toBeLessThanOrEqual(312)
    expect(bounds!.x).toBeGreaterThanOrEqual(0)
    expect(bounds!.x + bounds!.width).toBeLessThanOrEqual(320)
    await preview.getByRole('button', { name: 'View details' }).click()
    const drawer = page.locator('[data-usage-panel][data-usage-scope="all"]')
    ...
    await trigger.click()
    await preview.waitFor()
    await page.getByRole('navigation', { name: 'Mobile navigation' }).click({ position: { x: 1, y: 1 } })
  ...
  it('retains rest across rail and phone layouts and preserves four navigation destinations', async () => {
    ...
    for (const width of [1024, 768, 390, 320]) {
      ...
      if (width <= 768) {
        ...
        expect(await companion.getByRole('button').count()).toBe(2)
      }
