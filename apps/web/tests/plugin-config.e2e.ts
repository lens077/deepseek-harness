...
describe('web e2e: plugin configuration section', () => {
  ...
  it('shows one card per exposed host-plane namespace', async () => {
    ...
    expect(await dialog.getByText('文件锁', { exact: true }).count()).toBe(1)
    ...
    expect(await dialog.getByLabel('读取等待后询问（秒）').count()).toBe(0)
  ...
  it('stores a file-lock wait in the unit the Host keeps, not the one shown', async () => {
    ...
    const dialog = await openPlugins()
    await dialog.getByText('文件锁', { exact: true }).click()

    const readWait = dialog.getByLabel('读取等待后询问（秒）')
    ...
    expect(await dialog.getByLabel('写入排队上限（分钟）').inputValue()).toBe('10')

    await readWait.fill('5')
    await dialog.getByRole('radio', { name: '立即读取' }).click()
    await dialog.getByRole('button', { name: '保存', exact: true }).click()
