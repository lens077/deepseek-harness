...
async function openPersistedSession(page: Page, id: string): Promise<void> {
  ...
  const row = page.locator(`[data-session-id="${id}"][draggable="true"]`)
...
describe('web e2e: persisted usage and cost governance', () => {
  ...
  it('prices own requests, separates the subagent tree, and reports incomplete corpus totals', async () => {
    ...
    await page.locator(`[data-session-id="${ROOT_ID}"][draggable="true"]`).click()
  ...
  it('shows compact calendar usage from real dated projections and retains detailed scopes', async () => {
    await page.clock.setFixedTime(SEED_TIME + 60_000)
    const trigger = page.getByRole('button', { name: 'Usage overview', exact: true })
    await trigger.click()
    const preview = page.locator('[data-usage-preview]')
    await preview.waitFor()
    expect(await preview.locator('[data-usage-period]').getAttribute('data-usage-period')).toBe('today')
    expect(await preview.innerText()).toContain('UTC')
    expect(await preview.innerText()).not.toContain('Daily usage is not ready')
    await preview.getByRole('button', { name: 'This week', exact: true }).click()
    await preview.getByRole('button', { name: 'This month', exact: true }).click()
    await compareOrRefreshGolden(fileURLToPath(new URL('./expected/usage-governance/preview.expected.md', import.meta.url)),
      await captureStableAria(page, '[data-usage-preview]', scaffold.workspaceCwd), MODE)
    await preview.getByRole('button', { name: 'View details' }).click()
    const drawer = page.locator('[data-usage-panel]')
    await drawer.waitFor()
    expect(await drawer.getAttribute('data-usage-scope')).toBe('all')
    expect(await drawer.getByText(/1 requests have incomplete billing inputs/).count()).toBe(1)
    await drawer.getByRole('button', { name: 'This session', exact: true }).click()
    expect(await drawer.locator('[data-usage-cost]').textContent()).toContain('USD 0.00018')
    await drawer.getByRole('button', { name: 'Session tree', exact: true }).click()
    expect(await drawer.locator('[data-usage-cost]').textContent()).toContain('USD 0.00052')
